...
fn run_benchmark(monitor: &Monitor, quality: u8, label: &str) -> Vec<FrameTiming> {
    println!("\n▶ Quality {label}");
    println!("{:>5}  {:>8} {:>8} {:>9} {:>8}  {:>8}  {:>7}",
        "frame", "capture", "resize", "jpeg_enc", "base64",
        "total", "fps");
    println!("{}", "-".repeat(65));

    // Warm-up capture so WGC session is established before timing.
    for _ in 0..3 { let _ = monitor.capture_image(); }
    ...
    for i in 0..FRAMES {
        ...
        let rgba = match monitor.capture_image() {
            Ok(img) => img,
            Err(e) => { eprintln!("  frame {i}: capture failed: {e}"); continue; }
        };
        ...
        let timing = FrameTiming { capture_us, resize_us, encode_us, base64_us, jpeg_bytes, src_w, src_h };
        let total_us = timing.total_us();
        let fps = if total_us > 0 { 1_000_000.0 / total_us as f64 } else { 0.0 };

        println!("{:>5}  {:>6.1}ms {:>6.1}ms {:>7.1}ms {:>6.1}ms  {:>6.1}ms  {:>6.1}",
            i,
...
fn print_summary(timings: &[FrameTiming], quality: u8, label: &str) {
    ...
    let avg_us = |f: fn(&FrameTiming) -> u64| -> f64 {
        timings.iter().map(f).sum::<u64>() as f64 / n
    };
    let max_us = |f: fn(&FrameTiming) -> u64| -> f64 {
        timings.iter().map(f).max().unwrap_or(0) as f64
    };
    ...
    println!("\n── Summary  quality={quality} {label}  ({} frames) ─────────────────────────────", timings.len());
    println!("  Source resolution: {}×{}{}", src.0, src.1,
    ...
    println!("    ─────────────────────────────");
    println!("    TOTAL avg        {:7.2} ms  →  {:5.1} fps (pipeline-limited max)", avg_total / 1000.0, 1_000_000.0 / avg_total);
    println!("    TOTAL min        {:7.2} ms  →  {:5.1} fps", min_total / 1000.0, 1_000_000.0 / min_total);
    println!("    TOTAL max        {:7.2} ms  →  {:5.1} fps (worst case)", max_total / 1000.0, 1_000_000.0 / max_total);
    ...
    println!("  ⚠ Bottleneck:     {} ({:.2} ms avg = {:.0}% of pipeline)",
        bottleneck.0, bottleneck.1 / 1000.0,
    ...
    println!("    bottleneck stage  {:7.2} ms  →  {:5.1} fps (expected in-app fps)",
        pipeline_bottleneck_us / 1000.0, 1_000_000.0 / pipeline_bottleneck_us);
}
...
fn main() {
    ...
    for (quality, label, timings) in &all_summaries {
        ...
        println!("  quality={quality:3}  {label}  │  {:6.1}ms → {:5.1} fps  │  {:5.1} KB/frame",
            avg_total / 1000.0, 1_000_000.0 / avg_total, avg_jpeg_kb);
    }
    println!("╚═════════════════════════════════════════════════════════════════════╝");
    println!("\nDone.");
}
