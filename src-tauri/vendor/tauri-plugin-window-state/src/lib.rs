...
impl<R: Runtime> WindowExtInternal for Window<R> {
    fn update_state(&self, state: &mut WindowState, flags: StateFlags) -> tauri::Result<()> {
        let is_maximized = flags
            ...
            && self.is_maximized()?;
        // ALPINE PATCH: MAXIMIZED joins the flag set, because the maximized bit below now depends
        // on `is_minimized` being meaningful. It only widens the cases in which `is_minimized` is
        // read - the position and size guards it also feeds are themselves gated on POSITION/SIZE.
        let is_minimized = flags
            .intersects(StateFlags::MAXIMIZED | StateFlags::POSITION | StateFlags::SIZE)
            && self.is_minimized()?;

        // ALPINE PATCH: not while minimized. `is_maximized` is false for a window that was
        // minimized *from* maximized - on Windows, tao caches the bit and clears it on
        // WM_SIZE/SIZE_MINIMIZED - and this function runs on CloseRequested and on exit, so
        // minimizing a maximized window and then closing it wrote `maximized: false` while x/y
        // still held the maximized origin. Restoring that put the window at the top-left corner
        // of the monitor at whatever size predated the maximize, and because that *is* a plain
        // un-maximized window, closing it saved the same values straight back: the state was a
        // fixed point that no amount of moving the window could shake off.
        //
        // The bit is instead recorded as it changes, by the Resized handler in `Builder::build`.
        // Skipping it here leaves that value alone rather than overwriting it with a reading taken
        // at the one moment the platform is known to report it wrongly.
        if flags.contains(StateFlags::MAXIMIZED) && !is_minimized {
        ...
        PluginBuilder::new("window-state")
            ...
            .on_window_ready(move |window| {
                ...
                window.on_window_event(move |e| match e {
                    ...
                    // ALPINE PATCH: MAXIMIZED added to the guard, so the arm still runs for a
                    // caller that tracks the maximized bit without tracking size.
                    WindowEvent::Resized(size)
                        if state_flags.intersects(StateFlags::SIZE | StateFlags::MAXIMIZED) =>
                    {
                        if window_clone
                            .state::<RestoringWindowState>()
                            .0
                            ...
                            .is_ok()
                        {
                            ...
                            if !window_clone.is_minimized().unwrap_or_default() {
                                let mut c = cache.lock().unwrap();
                                if let Some(state) = c.get_mut(&label) {
                                    // ALPINE PATCH: the maximized bit is recorded here, as it
                                    // changes, rather than read once at close. Every maximize and
                                    // every restore-down arrives as a resize, and this reading -
                                    // unlike the one `update_state` used to take - can never be
                                    // taken while the window is minimized, which is the case the
                                    // platform gets wrong. See the `update_state` patch.
                                    if state_flags.contains(StateFlags::MAXIMIZED) {
                                        state.maximized = is_maximized;
                                    }

                                    if state_flags.contains(StateFlags::SIZE) && !is_maximized {
                                        state.width = size.width;
                                        state.height = size.height;
                                    }
                                }
