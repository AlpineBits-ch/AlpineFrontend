...
impl FaceBsp {
    ...
    fn edges_for_faces(faces: &[Face]) -> Vec<(usize, usize)> {
        ...
        //
        // ALPINE PATCH: `wrapping_add`, not `+`. An LCG is meant to wrap - the multiply already
        // says so with `overflowing_mul` - but the addition was left unguarded, so with debug
        // assertions on this panics with "attempt to add with overflow" for any edge count whose
        // product lands in the top ~24% of u32. It is not a rare edge case: it depends only on the
        // mesh, so a given HRIR sphere either always panics or never does. Release builds wrap
        // silently and work, which is why it survives upstream. `HrirSphere::new` runs at runtime
        // when a sphere is loaded, so unpatched this crashes any debug build of the app that turns
        // spatial audio on.
        let first_idx = (edges.len() as u32)
            .overflowing_mul(1664525)
            .0
