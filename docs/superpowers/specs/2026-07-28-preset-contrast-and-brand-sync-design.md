...
`login.component.html` also uses Tailwind's *built-in* `indigo-400`/`indigo-700` gradient utility classes and a literal `rgba(67,56,202,0.45)` shadow (rgb form of old `brand-dark`) — replace with arbitrary-value classes referencing our vars, e.g. `from-[var(--color-brand-dim)] to-[var(--color-brand-dark)]` and `shadow-[0_4px_18px_color-mix(in_srgb,var(--color-brand-dark)_45%,transparent)]`.
...
- Grep the repo for the old indigo hex/rgb values (`6366f1|4f46e5|818cf8|4338ca|99,\s?102,\s?241|129,\s?140,\s?248|67,\s?56,\s?202`) and for the Tailwind built-in utility classes (`indigo-400|indigo-700`) — zero remaining hits outside the intentional role-swatch-default literals (now `#7c72ff`).
