...
import {VoiceEngineService} from './app/services/voice-engine.service';
import {IsleVoiceRtcService} from './app/services/isle-voice-rtc.service';

/** The globals this file hangs on `window`. Development builds only; nothing may depend on them. */
interface DebugWindow {
    __expireToken: () => void;
    __expireTokenConcurrent: (n?: number) => void;
    __fireTokenExpiresEvent: () => void;
    __showTokenState: () => void;
    __voiceStats: (seconds?: number) => Promise<void>;
    __isleSubs: () => void;
}
...
export function registerDebugHelpers(appRef: ApplicationRef): void {
    ...
    const globals = window as unknown as DebugWindow;
    ...
    globals.__expireTokenConcurrent = (n = 3): void => {
        localStorage.setItem(TOKEN_KEY, 'debug.expired.token');
    ...
    globals.__fireTokenExpiresEvent = (): void => {
        // eventsSubject is protected in the library but accessible at runtime.
        const subject = (oAuth as unknown as {eventsSubject?: Subject<OAuthEvent>}).eventsSubject;
        if (!subject) {
            console.error('[debug] eventsSubject not found on OAuthService: library version mismatch?');
            return;
        }
        subject.next({type: 'token_expires'} as OAuthEvent);
    ...
    globals.__voiceStats = async (seconds = 2): Promise<void> => {
