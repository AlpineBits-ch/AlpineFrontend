/** What a call surface says about itself, as one ranked condition rather than a pile of banners. */
...
export interface CallStatusInput {
    ...
    /** Formatted wall-clock time the server will end the call at. Null when no countdown is running. */
...
/** Picks the one condition worth reporting. Total: `connected` is the floor, never an absence. */
...
function resolveKind(input: CallStatusInput): CallStatusKind {
    // `failed` is terminal and `disconnected` is not, so they stay separate lines.
