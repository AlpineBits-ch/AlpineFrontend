...
function setup(s: CallScreenShare): ComponentFixture<CallShareTileComponent> {
    TestBed.configureTestingModule({
        ...
        // RustMediaService is stubbed: the real one reaches for ScreenPublisher.
        providers: [
            {
...
/** The pan/zoom surface, which the template renders as the tile root's first child. */
...
describe('CallShareTileComponent pop-out', () => {
    ...
    beforeEach(() => {
        ...
        // The harness does not set the theme class index.html puts on `body`.
        document.body.className = 'dark';
        // jsdom implements neither play() nor pause().
        HTMLMediaElement.prototype.play = vi.fn(() => Promise.resolve());
        HTMLMediaElement.prototype.pause = vi.fn();

        // A rule nothing else could have written, so "styles were copied" is not just a count.
...
describe('CallShareTileComponent PiP route selection', () => {
    ...
    beforeEach(() => {
        ...
        HTMLMediaElement.prototype.play = vi.fn(() => Promise.resolve());
        HTMLMediaElement.prototype.pause = vi.fn();
        // jsdom has no video PiP, so this is the whole implementation for the fallback route.
