...
describe('downloadErrorStatus', () => {
    it('reads the status of an HttpErrorResponse', () => {
        expect(downloadErrorStatus(new HttpErrorResponse({status: 410, statusText: 'Gone'}))).toBe(410);
    });

    it('reads the status the native command serialises', () => {
        expect(downloadErrorStatus({status: 409, message: 'export download answered 409 Conflict'})).toBe(
            409,
        );
    });
    ...
    it('reports no status for a transport failure from either shell', () => {
        expect(
            downloadErrorStatus(new HttpErrorResponse({status: 0, statusText: 'Unknown Error'})),
        ).toBeNull();
