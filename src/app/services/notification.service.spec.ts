/**
 * `NotificationService` is now a delegate over the {@link Notifier} port: it decides *whether* a
 * notification is warranted - category filters, sound, cooldown - and the adapter decides how one is
 * shown. These tests are about the seam, and above all about the one failure a browser makes possible
 * and desktop never did: **a user seeing notifications reported as on while nothing is being shown.**
 *
 * <p>No `vi.mock` of a Tauri module anywhere. A fake adapter is provided in TestBed instead, which is
 * what lets the web-shaped cases - a durable permission refusal, a host with no native toasts - be
 * expressed at all.</p>
 */
...
/**
 * <p><b>The failure that matters.</b> `notificationSettings().enabled` is the user's *intent* and stays
 * true; whether the host will show anything is a separate fact, and if the two are never reconciled the
 * settings page reports notifications as on while the user gets none. `notificationsBlocked` is the
 * reconciliation, and it is latched only where a refusal is durable.</p>
 */
describe('when permission is refused', () => {
    ...
    it('reports itself blocked on a host that will not re-prompt, and stops asking', async () => {
        ...
        // A browser resolves `denied` without showing the user anything, forever. Asking per message
        // is a no-op that hides the state instead of surfacing it.
    ...
    it('keeps asking on a native host, where a refusal is not final', async () => {
        // The user can grant notifications in OS settings without the app ever hearing about it, so a
        // desktop refusal has to be re-checked. Latching here would silently disable notifications
        // for the rest of the session after one early "not now".
        const {service, notifier} = setup({
            configure: n => {
                n.permissionGranted = false;
            },
    ...
    it('still plays the sound, because that needs no host permission', async () => {
        // Sound is the app's own audio element. Losing it along with the toast would make a denied
        // browser silent in both senses for no reason.
