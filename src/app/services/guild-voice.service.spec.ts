...
/** The gateway rewrite: one singular `guild` segment, then the plural resource. A path the gateway does not recognise 404s silently. */
...
describe('GuildVoiceService', () => {
    describe('connection', () => {
        ...
        it('omits tag entirely rather than sending an empty one', () => {
            // A blank tag is not no tag: the server falls back to `alt`, so an always-present
            // `&tag=` would hand every tagless secondary connection the same identity.
            const {service, ctrl} = setup();
