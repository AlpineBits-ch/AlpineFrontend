 ...
 *
 * <p>The round-trip through JSON is the point: it is what drops `undefined` keys, so asserting on
 * it catches a field that was meant to be omitted but is actually being sent as `null`, and the
 * reverse. Every one of these DTOs was written against an inferred shape, and the wire is the only
 * place the inference is checkable.</p>
 */
...
// ── Wire shapes ─────────────────────────────────────────────────────────────
// Asserted against `Guild.Application.Dtos.Request.CreateDecisionDto` and
// `CastDecisionVoteDto` rather than against what the client happens to build, because the whole
// module was written before either was readable.
...
describe('DecisionService request bodies', () => {
    it('POSTs a create with the option titles as a bare string list', async () => {
        ...
        // Options are titles, not objects: position is assigned server-side from this order, and
        // sending `{title}` objects would deserialize to two empty strings.
    ...
    it('sends an explicit null for the two optional fields rather than omitting them', async () => {
        ...
        const req = http.expectOne(`${GUILD}/channels/chan_1/decisions`);
        // Both readings are legal - the server's fields are nullable - so this pins the one the
        // client actually uses, which is the thing a future refactor can silently change.
        expect(wireBody(req.request.body)).toEqual({
            title: 'Cat?',
            description: null,
            closesAt: null,
    ...
    it('PUTs a Support carrying its optionId and nothing else', async () => {
        ...
        const req = http.expectOne(`${GUILD}/decisions/deci_1/vote`);
        expect(req.request.method).toBe('PUT');
        // `kind` is the enum *name*: Guild.Application registers a JsonStringEnumConverter, so an
        // ordinal would be rejected outright.
    ...
    it('PUTs an Abstain with neither an option nor a reason', async () => {
        ...
        const req = http.expectOne(`${GUILD}/decisions/deci_1/vote`);
        // The server nulls an abstain's OptionId regardless; sending one anyway would say the
        // client thinks abstaining is a vote for something.
