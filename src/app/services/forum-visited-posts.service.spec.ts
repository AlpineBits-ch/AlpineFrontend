...
/**
 * This runner's global localStorage exists but has no methods on it, so the service's own
 * try/catch swallows every read and persistence could never be asserted. An in-memory
 * stand-in makes the storage behaviour testable instead of invisible.
 */
...
describe('ForumVisitedPostsService', () => {
    ...
    /**
     * The effect fires on every main-view change, so it has to be able to tell a forum
     * post from everything else the user opens.
     */
