...
function setup() {
    ...
    // Explicit, so a test that throws does not turn every later one into "already instantiated".
    ...
    TestBed.configureTestingModule({
        providers: [
            ...
            // Plaintext previews never reach the decryptor's MLS calls.
            ...
            // The real one reaches the OS notification bridge in its constructor.
...
/** Drains the microtask queue: `open` is fire-and-forget, and a single `Promise.resolve` is not enough. */
const settle = () => new Promise(resolve => setTimeout(resolve, 0));

/** Fetches the summary and settles it. The flush must happen while the call is still in flight. */
...
describe('InboxService', () => {
    ...
    describe('paging', () => {
        it('keeps paging when a page is empty but still has a cursor', async () => {
            ...
            // Filtering is applied after the page is taken, so an empty page with a cursor means "keep going".
    ...
    describe('waiting on you', () => {
        ...
        it('counts tasks in the badge', async () => {
            const {service, ctrl} = setup();
            // A list channel holds no messages, so nothing else would ever light the badge for it.
        ...
        it('keeps the task count through a mark-all-read', async () => {
            ...
            // A chore that is due is still due after `read-all`; zeroing it clears a badge the server puts back.
