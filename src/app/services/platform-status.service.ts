 ...
 *
 * <p>Two, not one. A single failed call is a blip - a dropped request on a train, a laptop coming
 * out of sleep - and the spec is right that "could not load status" is noise. Two in a row is a
 * client that genuinely cannot reach the one endpoint built to always answer, and at that point
 * saying so beats a silent, confidently-empty status bar.</p>
 */
const UNVERIFIED_AFTER_FAILURES = 2;

/**
 * How soon the first failure is retried.
 *
 * <p>Waiting a full {@link POLL_INTERVAL_MS} for the second opinion would mean a user staring at a
 * dead app for a minute before being told we cannot check - which is most of the value of telling
 * them at all. One quick confirmation, then back to the normal cadence.</p>
 */
const CONFIRM_FAILURE_DELAY_MS = 5_000;

/**
 * How long the socket may stay down before it counts as "the first reconnect attempt failed".
 *
 * <p>SignalR's automatic reconnect gives no per-attempt callback - `onreconnecting` fires once per
 * drop, `onclose` never fires at all while the retry policy keeps returning a delay - so the state
 * signal plus a delay is the only available reading of it. The first retry goes out at 1 s and the
 * second at 2 s, so still-not-connected at 4 s means at least one attempt has been made and lost.</p>
 */
...
/**
 * "Is it me, or is it them" - held for the whole app, signed in or not.
 *
 * <p>One {@link StatusApiService.summary} call drives the banner, the settings page and the
 * per-feature hints. It is polled every 60 s while the window is in the foreground and never on a
 * timer that survives backgrounding: a million idle clients polling a status endpoint is its own
 * outage. It is also fetched once, out of band, at the exact moments the answer is worth something
 * - the API is unreachable, a sign-in died on a 5xx, the socket will not come back - via
 * {@link probe}.</p>
 *
 * <p><b>One deliberate deviation from the platform spec.</b> The spec says to render nothing when
 * the status call itself fails. This shows a quiet, dismissible "could not verify" bar after
 * {@link UNVERIFIED_AFTER_FAILURES} consecutive failures instead, because a user staring at a
 * broken app is better served by "we cannot tell you" than by a blank space that reads as "all
 * fine" - and the copy is careful to say exactly that and no more.</p>
 *
 * <p>Every failure counts the same, `404` included. A server that does not answer
 * `/api/v1/status/summary` - not deployed yet, routes missing, a self-hosted build that predates
 * the endpoint - is precisely a server whose status this client cannot verify, and saying so is
 * true. An earlier draft special-cased `404` as "this instance has no status API" and went silent,
 * which is the one outcome the bar exists to prevent.</p>
 */
...
export class PlatformStatusService {
    ...
    constructor() {
        // Watched rather than subscribed to from the realtime service, because the dependency has
        // to point this way: `RealtimeConnectionService` reaches `AuthService`, so a callback in
        // the other direction closes an injection cycle.
        ...
        effect(() => {
            ...
            // Only a *drop* counts. A signed-out client never connects the hub at all, and
            // treating its permanent `Disconnected` as a reconnect failure would probe on every
            // visit to the login screen for no reason.
    ...
    /**
     * What the top-of-app bar renders, or null for nothing.
     *
     * <p>A real incident outranks "we could not check": if we are holding a summary that says
     * something is wrong, that is more useful than the fact that the latest refresh failed.</p>
     */
    readonly bar = computed<StatusBarKind | null>(() => {
        const banner = this.banner();
        // Driven off the banner's presence rather than `indicator !== 'operational'`: the server
        // only sends one when there is something to say, and an indicator value this build has
        // never heard of must not decide whether the user sees a bar.
    ...
    /**
     * `postedAt` of the newest update on the banner's incident, which is the half of the dismissal
     * key that makes a *new update on an already-dismissed incident* re-show the bar.
     */
     ...
     * an unreachable API, a sign-in that died on a 5xx, a socket that will not reconnect.
     *
     * <p>Cheap to over-call - a fetch already in flight wins and this returns immediately - so
     * call sites do not have to coordinate.</p>
     */
    probe(): void {
        this.refresh();
    }
    ...
    /**
     * Hides the current bar until it becomes new information again.
     *
     * <p>Keyed on the incident reference *and* the newest update's timestamp, so an update posted
     * after the user dismissed the bar brings it back - that update is precisely what they asked to
     * be told about by opening the app at all.</p>
     */
    dismiss(): void {
        const kind = this.bar();
        if (kind === 'unverified') {
    ...
    /**
     * Merges one incident from `status.IncidentUpdated`.
     *
     * <p>An incident we have never seen means our summary predates it, and the summary carries the
     * banner copy this event does not - so that case refetches rather than guessing at a banner.</p>
     */
    ...
    private wireRealtime(): void {
        ...
        // signed-out user is quite possibly signed out *because* of the incident.
    ...
    private writeDismissal(dismissal: Dismissal | null): void {
        ...
        try {
            ...
            // A storage that refuses to write costs the dismissal surviving a restart, nothing more.
        }
    }
