...
describe('MlsService.importBackup', () => {
    ...
    beforeEach(() => {
        ...
        TestBed.configureTestingModule({
            providers: [
                ...
                // Pinned: key-store entry names are `alpine_mls_{deviceId}_*`, which is what this asserts.
    ...
    it('writes the restored signing keypair to the OS keychain', async () => {
        ...
        // The signing keypair must reach the key store, not only the session: `autoUnlock` reads it
        // out of the keychain on every cold start.
    ...
    it('restores the message cache under the keys the export carried', async () => {
        ...
        // Composite keys survive as composite keys: re-deriving them files one conversation's
        // plaintext under another's whenever the guessed context lands wrong.
    ...
    it('reports what §D actually restored rather than deciding again on this side', async () => {
        ...
        // The device id alone decides what a restore recovers, and it is enforced in Rust; a second
        // opinion on this side would clone ratchet state onto every new device.
    ...
    //
    // `#floor` is the monotonic encryption floor, written once and never deleted. Lowering it by
    // restoring an older backup sends the very next message into an encrypted context in the clear.
    ...
    describe('encryption floor', () => {
        ...
        it('does not move the active generation back below the floor', async () => {
            ...
            // A restored "currently on generation 2" is stale information about the present, not evidence.
        ...
        it('still restores the per-generation group mappings from an older backup', async () => {
            ...
            // Additive by construction: two eras of one context are different groups, so refusing the
            // downgrade must not cost the history the backup was restored for.
    ...
    describe('account identity key', () => {
        ...
        it('writes nothing when only one half is present', async () => {
            ...
            // A public key with no private half is not a usable identity; storing it would make the
            // next export emit a broken `accountIdentity` section instead of a missing one.
            const written = setItem.mock.calls.map(([k]) => k);
            expect(written.some(k => k.includes('account_identity'))).toBe(false);
        });
    });
    ...
    //
    // The remedies are disjoint: an argument rejection reported as a credential rejection sends
    // someone hunting for a password that was never the problem.

    const cases: [string, string][] = [
        ['MlsError: not a backup file: EOF while parsing', 'not-a-backup'],
        // Two versions, two remedies, and only one of them is "update". Asserted in mls.rs too.
