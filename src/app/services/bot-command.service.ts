...
export class BotCommandService {
    ...
    /** Resolves when the bot's reply lands in the channel, or errors with {@link BotCommandTimeoutError}. The API carries no correlation id, so this matches only on (channelId, botUserId) and a new wait for the same pair cancels the previous one. */
