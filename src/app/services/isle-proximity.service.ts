...
/** Public surface for Isle proximity voice: detection, the join/leave flow, hotkey wiring and hub-reconnect resilience. The outgoing mic has exactly one gate, {@link syncMic}, and nothing else may touch `rtc.setMicEnabled`. */
...
export class IsleProximityService {
    ...
    /** Mic is open right now, derived by {@link syncMic} and never set directly. */
    ...
    constructor() {
        ...
        // Withdrawing positional-voice consent takes effect immediately, and must be mirrored locally or the mic stays open.
        // Only tears down on a real false: the flag reads permissive while the record is unknown, so a settings reload cannot drop a live call.
        effect(() => {
        ...
        // Keep the live spatial graph in sync with settings while connected. Output-device
        // changes apply instantly via AudioContext.setSinkId: no reconnect, no dropped peers.
        effect(() => {
        ...
        effect(() => {
            ...
            if (state === ConnectionState.Connected && prev !== ConnectionState.Connected) {
                // Reconnected: the backend may have forgotten state, so re-publish.
                void this.republish();
    ...
    async join(): Promise<void> {
        ...
        // Declining positional capture (T2-19) stops registration for the positional path entirely, not ordinary voice channels.
     ...
     *
     * @param playLeaveSound emit the voice-leave cue, used when the game disconnects out from
     *   under us so the drop is audible, rather than for a deliberate leave (which is silent).
     */
    ...
    /** Flip the mute toggle: the bar's mic button, and the hotkey in toggle mode. Mute is deliberate, so it survives leaving and rejoining. */
    toggleMute(): void {
    ...
    private startDetection(): void {
        ...
        // reconnect path cannot cover this; without a re-publish we stay audible to nobody.
    ...
    private refreshStatus(): void {
        ...
        // Reuses this tick rather than a timer of its own: a peer whose pull exhausted its retries has no other route back, because the relay will not order us again while the pair stays audible.
    ...
    /** Re-run the full publish flow. Guarded because a hub reconnect and an `isle.RepublishVoice` nudge can fire together for one server restart and tear down each other's fresh session. */
