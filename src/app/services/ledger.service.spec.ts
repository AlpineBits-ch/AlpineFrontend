...
/**
 * The body as it reaches the server, not as it was handed to HttpClient. The JSON round-trip drops
 * `undefined` keys, so an assertion on it catches an omitted field being sent as `null`.
 */
...
/** The one expense request in the air, matched on path alone: the URL carries a query string. */
...
describe('LedgerService', () => {
    ...
    describe('realtime', () => {
        ...
        it('re-fetches balances on a settlement, which touches no expense at all', () => {
            ...
            // The expense list is untouched, which is why the balance cannot be derived from it.
            expect(service.stateFor(CHANNEL).expenses.length).toBe(1);
    ...
    describe('writes', () => {
        ...
        it('writes the currency through the config route and relabels nothing else', () => {
            ...
            // The stored integer is untouched: 1234 rappen became 1234 cents, which is why changing
            // this needs a confirmation dialog.
            expect(service.stateFor(CHANNEL).expenses[0].amountMinor).toBe(1234);
        });
    });

    // ── Wire shapes ──────────────────────────────────────────────────────────
    // Asserted against the request DTOs rather than against what the client happens to build.
    ...
    describe('request bodies', () => {
        it('posts an Equal split as an empty shares array, which means everyone in the guild', () => {
            ...
            // sending an explicit payer would need ManageLedger.
        ...
        it('posts a Shares split as weights, never as per-person amounts', () => {
            ...
            // distributes the remainder, and a client that re-derives it eventually disagrees.
            expect(wireBody(req.request.body)).toEqual({
                description: 'Internet',
                amountMinor: 6000,
                payerUserId: 'user_anna',
        ...
        it('puts the currency on its own - the config body has exactly one field', () => {
            ...
            const req = ctrl.expectOne(`${GUILD}/channels/${CHANNEL}/ledger/config`);
            expect(req.request.method).toBe('PUT');
            // `UpdateLedgerConfigDto` is `{Currency}` and nothing else; the route carries the id.
...
describe('normalizeCurrencyCode', () => {
    ...
    it('rejects everything the server would answer 400 for', () => {
        // Three ASCII letters exactly: the server checks IsAsciiLetterUpper after upper-casing.
