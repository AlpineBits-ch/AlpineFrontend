 ...
 *
 * <p>`saveToDisk` streams the artifact through the native HTTP client because the endpoint answers `302`
 * to a signed Google Cloud Storage URL that serves no `Access-Control-Allow-Origin` - a webview fetch is
 * blocked by CORS for an artifact that is sitting there intact. There is therefore no browser
 * implementation to write, and none is faked: <b>the web answer is a capability gate and a typed
 * refusal</b>, never a silent no-op, which is the bug class the capability rules exist to prevent.</p>
 *
 * <p>No `vi.mock('@tauri-apps/api/core')`: the host is behind {@link OsInfo}, so both branches are
 * reachable by providing a different fake, which is what makes the browser branch testable at all.</p>
 */
...
describe('DataExportService.canSaveToDisk', () => {
    ...
    /**
     * Both halves of the original gate still matter. The Rust command exists in the mobile shell too, so
     * this is not redundant with the host check - a phone simply has no file picker worth showing.
     */
...
describe('DataExportService on a host without the native path', () => {
    ...
    /**
     * The refusal carries the {@link DataExportDownloadError} shape, so it flows through the existing
     * error reporting rather than needing a branch of its own. `status: null` is honest - nothing
     * answered, because nothing was asked - and lands on the generic message.
     */
