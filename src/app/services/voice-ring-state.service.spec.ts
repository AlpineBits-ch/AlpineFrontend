...
describe('VoiceRingStateService incoming', () => {
    ...
    it('drops a ring that is already dead on arrival', () => {
        // A push that sat in a queue longer than the invitation lived. Drawing it would be drawing
...
describe('VoiceRingStateService resolution', () => {
    ...
    it('says nothing when this very device is the one that answered', async () => {
        ...
        // Resolved by us: we already rendered whatever we did, and re-announcing it would tell
        // somebody they declined a second after they pressed decline.
        ws.voiceRingResolvedObservable.next({
            ringId: 'ring_1', guildId: 'g1', channelId: 'chan_1',
...
describe('VoiceRingStateService sending', () => {
    ...
    it('never renders a block as "you are blocked"', () => {
        // The server deliberately does not say which direction the block runs in, so neither may
        // the copy - "they blocked you" would be a guess that is wrong half the time.
