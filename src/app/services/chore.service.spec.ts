...
/** The body as it reaches the server: the JSON round-trip drops `undefined`, and "omitted" and "sent as null" are different requests. */
...
describe('ChoreService realtime registration', () => {
    it('registers each of the five chore events exactly once', () => {
        setup();
        // `RealtimeConnectionService.on` does not deduplicate, so a second registration delivers
        // every event twice, and a doubled occurrence event means two balance refetches.
        for (const event of [
            'guild.ChoreCreated',
            'guild.ChoreUpdated',
            'guild.ChoreDeleted',
    ...
    it('leaves the due reminder alone - it is a household alert, and this service is opened into', () => {
        setup();
        // This service is constructed by the chores board, so a listener here would only reach
        // somebody who had already opened the board they are being reminded about.
...
describe('ChoreService loading', () => {
    it('flags a 403 separately from an ordinary failure', () => {
        ...
        // A 403 here usually means the guild's Chores module is off, not a missing permission bit,
        // and the UI has to be able to say the right sentence.
...
describe('ChoreService verbs', () => {
    ...
    it('does not guess who a swap lands on', () => {
        ...
        // Still Anna's until the server answers: guessing from a possibly stale balance would name
        // the wrong flatmate.
        expect(row(service).assignedUserId).toBe('user_anna');
...
// ── Wire shapes ─────────────────────────────────────────────────────────────
// Asserted against `Guild.Application.Dtos.Request.CreateChoreDto` / `UpdateChoreDto`.
...
describe('ChoreService request bodies', () => {
    ...
    it('has no anchorAt to send on an update - UpdateChoreDto does not carry one', () => {
        ...
        // the dialog exactly like a re-phase that worked.
