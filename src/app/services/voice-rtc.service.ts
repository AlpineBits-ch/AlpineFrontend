...
export class VoiceRTCService {
  ...
  cleanupParticipant(userId: string): void {
    ...
    if (audio) { audio.pause(); audio.srcObject = null; this.remoteAudioEls.delete(userId); }

    const screenAudio = this.remoteScreenAudioEls.get(userId);
    if (screenAudio) { screenAudio.pause(); screenAudio.srcObject = null; this.remoteScreenAudioEls.delete(userId); }

    const vad = this.vadHandles.get(userId);
    if (vad) { clearInterval(vad); this.vadHandles.delete(userId); }

    this.videoStreamsSignal.update(m  => { const n = new Map(m);  n.delete(userId); return n; });
    this.screenStreamsSignal.update(m => { const n = new Map(m);  n.delete(userId); return n; });
    this.screenAudioMutedSignal.update(s => { const n = new Set(s); n.delete(userId); return n; });
    this.participantsWithAudio.update(s => { const n = new Set(s); n.delete(userId); return n; });
  }
  ...
  handleRemoteTrackClosed(trackName: string, userId: string): void {
    if (trackName === 'video') {
      ...
      const audio = this.remoteScreenAudioEls.get(userId);
      if (audio) { audio.pause(); audio.srcObject = null; this.remoteScreenAudioEls.delete(userId); }
