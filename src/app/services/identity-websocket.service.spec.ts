/** The `identity.*` events. Nothing here writes group state, and every boolean fails towards the alarming answer. */
    ...
    TestBed.configureTestingModule({
        providers: [
            ...
            {provide: MasterKeyStateService, useValue: masterKeyState},
            {
                ...
                // Returns the key, so an assertion can name the string without the locale submodule.
...
describe('identity.DeviceRegistered', () => {
    ...
    // `identity.AccountIdentityKeyRotated` is a different endpoint, so anything that stops warning here goes silent.
...
describe('identity.DeviceAdmitted', () => {
    ...
    it('reads a missing autoAdmitted as "nobody reviewed this"', async () => {
        ...
        // Defaulting the flag to false would invent a review that never happened.
        expect(seen[0].autoAdmitted).toBe(true);
    });
});
