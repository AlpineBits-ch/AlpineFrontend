 ...
 *
 * Ports, not module mocks: command names and argument objects are identical on the desktop and
 * browser adapters, so every assertion here has to stay true of both hosts.
 */
...
// ---------------------------------------------------------------------------
// The engine, as a provided fake: `invokeStub.mock.calls` holds the command name at [0] and the
// argument object at [1].
// ---------------------------------------------------------------------------
...
function platformProviders() {
    ...
    return [
        ...
        // Pinned: the real one drags `AccountRegistryService` and the settings store into this
        // injector, and none of that is what this file is about.
        {
...
describe('MlsService', () => {
    ...
    describe('initStorage', () => {
        ...
        it('passes the state key so the file is never written in the clear', async () => {
            mockInvoke(true);
            await firstValueFrom(service.initStorage());
            // Both fetched inside the service, never taken as parameters: a caller could forget the
            // key and leave private keys on disk as plain JSON, or share one state file across accounts.
    ...
    // The local state key
    //
    // The licence to mint a state key is tied to a successful read that reported absence, and to
    // nothing else: minting on a read that failed hands the engine the wrong key, the sealed state
    // fails to authenticate, and the launch wipes every group on the device.
    // ─────────────────────────────────────────────────────────────────────────

    describe('the local state key', () => {
        const STATE_KEY_NAME = `alpine_mls_${DEVICE_ID}_statekey`;

        /** The fake store is a module-level singleton, so a key minted by an earlier test would otherwise be found by the next one. */
        beforeEach(async () => {
        ...
        /** The one falsy value still minted over: an empty string cannot be a 32-byte base64 key under any encoding. */
    ...
    // autoUnlock
    //
    // `KeyNotFound` must mean "the store was read, three times, and reported all three entries
    // missing", and nothing else: it routes to registration, which mints a fresh keypair over these
    // entries and ejects the device from every group it belongs to.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('autoUnlock', () => {
        ...
        /** The fresh-device direction: if this stopped answering `KeyNotFound`, no new device could set encryption up. */
        ...
        /** A failed read of one entry is not an absent entry: a truthiness test cannot tell them apart and calls the device unregistered. */
        it('never says KeyNotFound when one of the three reads failed', async () => {
            ...
            expect(err.kind).toBe('MlsError');
            // Names the entry: "secure storage is unavailable" with no subject is not evidence.
        ...
        /** A total read failure and a scoped one must land on the same kind. */
        ...
        /** The three entries are written and deleted non-atomically, so a partial store is reachable with no fault at all, and it is not "not registered". */
        it.each([
        ...
        /** An empty string is not absence here, the opposite call from `localStateKey`: these three entries are this device's identity in every group. */
        ...
        /** Fold `''` into absence and a device with three keychain entries is reported as never registered. */
        ...
        /** The launch, driven by the real `autoUnlock` over the fake store. No non-absence refusal may set `needsRegistration`. */
        ...
        it('never reaches the registration modal on a partly-stored key', async () => {
            ...
            // Its own flag, not the transient one: the reads succeeded and "try again" is a lie.
            expect(outcome.keyStoreUnreachable).toBe(false);
            expect(secureStore.peek(PUB)).toBe('cHVia2V5');
            expect(secureStore.peek(PRIV)).toBe('cHJpdmtleQ==');
        });

        /** The fresh device must still get the modal, or nobody can set encryption up on a new install. */
    ...
    describe('full MLS group lifecycle', () => {
        ...
        it('generate → replenish → create → add → join → send → receive → remove → leave → delete', async () => {
            ...
            // Step 5: Alice adds Bob, producing a commit and a welcome
            ...
            // Step 6: Bob joins via the welcome and sees both members in the roster
            ...
            // Step 8: Bob receives the message. No roster re-check: openmls already resolved the
            // sender from an in-tree leaf and verified the signature.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Message cache at rest: it holds the plaintext of every message this device has ever read.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('message cache', () => {
        ...
        /** The cache file this account resolves to. It is named per device id, never per installation, or a second account can open the first one's plaintext history. */
        ...
        //
        // `messageId` is the server's. Keyed on it alone, a replayed id renders one thread's
        // plaintext inside another, and a cache hit skips every author binding on this client.
        ...
        it('writes the exact composite key venta-mobile writes', async () => {
            ...
            // bare-id entries have to drain on both platforms in step.
            await service.cacheMessage(CTX_A, 3, 'msg-1', 'aGVsbG8=');
        ...
        it('reads an entry written before the cache was sealed', async () => {
            // Bare base64 is what shipped. Discarding those entries throws away the only copy of
            // that message's plaintext: MLS decrypts from the wire exactly once.
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // C1: the monotonic encryption floor. It only ever rises, and only an explicit local disable
    // lowers it, or a server answering `{encrypted: false}` sends the next message in the clear.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('encryption floor', () => {
        ...
        it('survives clearing the active generation', async () => {
            ...
            // `clearActiveGeneration` is what a server-reported downgrade triggers; the floor is
            // the local proof that outlives it.
        ...
        /** The registry readers answer null with no engine rather than rejecting: send and receive both read the floor uncaught, so a rejection kills both paths. */
        it('answers null rather than throwing when there is no engine', async () => {
            await service.registerGroup(CTX, 2, 'Z3JvdXA=');
            // The engine, not the host: `MlsEngine.available` is not false merely because the
    ...
    describe('with no engine', () => {
        ...
        it('refuses every engine call rather than reporting success', async () => {
            // "It crashes earlier" is not an access control: without this, a build that got this
            // far reports success for operations that never happened.
            engine.available = false;
            try {
...
// ─────────────────────────────────────────────────────────────────────────────
// Commands this build may not define: `mls_drain_pending_messages`, `mls_export_backup` and
// `mls_import_backup` have a TypeScript caller and no Rust definition, and an unresolved command
// must degrade rather than throw. Unit tests mock the IPC boundary, so nothing else catches this.
// ─────────────────────────────────────────────────────────────────────────────
