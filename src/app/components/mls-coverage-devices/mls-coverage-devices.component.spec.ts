...
/**
 * Renders against the real `en.json` rather than raw keys: half of what this component decides is
 * which of five near-identical strings to interpolate into, and a test that only sees the key
 * cannot tell "Alex's iPhone 15" from "Alex's device".
 */
...
describe('MlsCoverageDevicesComponent', () => {
    ...
    /**
     * Opaque, and still an answer. The reader opened this screen to find out which devices are
     * outside, and the row is about the device either way.
     */
    ...
    /** Fetching belongs to the screen, so a host can gate the section on hasDeviceReport without
     * deadlocking against a fetch that rendering would have started. */
