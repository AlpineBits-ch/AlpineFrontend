/**
 * The review surface for MLS admission requests in a conversation.
 *
 * <p>Every case here is one the live report walked through: a desktop submitted a §B join request
 * for an encrypted DM, the server pushed it to every member, and no client anywhere had a surface
 * that asked a human about it - so the request stayed Pending until it expired and the desktop
 * could never read the conversation. `approve()` existed and was correct; it had exactly one
 * caller, in guild channel settings.</p>
 */
...
function setup(options: {
    ...
    /** Null means this device holds no group for the context - it can admit nobody. */
    ...
    return {
         ...
         *
         * <p>A macrotask rather than `whenStable`: the refresh is a plain promise chain over the
         * device store and the queue read, which zoneless change detection does not track, so
         * stability says nothing about whether it has landed.</p>
         */
        settle: async () => {
...
describe('MlsJoinRequestReviewComponent', () => {
    ...
    it('shows the fingerprint and never the key-package hash', async () => {
        ...
        // and would learn that a mismatch is normal, which is the opposite of the point.
    ...
    it('words another of your own devices as a different question from a correspondent\'s', async () => {
        ...
        // The trust question is answerable by walking to the other machine, and the wording has to
        // say so - "check with Bob out of band" is nonsense when Bob is you.
    ...
    it('offers nothing when this device holds no group and could not admit anyone', async () => {
        ...
        // Admission is an Add commit and only a device holding the group can produce one. A device
        // that is itself locked out - a very likely thing to be looking at a piling-up queue -
        // would otherwise be shown a button whose every press ends in "no group found".
    ...
    it('picks up a request that arrives while the conversation is open', async () => {
        ...
        expect(text()).toContain('waiting to be let into this conversation');
        // The push is the only place a device name appears at all - the queue read omits it.
    ...
    it('says the queue could not be read instead of showing an empty one', async () => {
        ...
        // A failed read is not "nobody is asking", and rendering it as such is the silence this
