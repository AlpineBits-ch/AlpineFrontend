...
/** Where the flow is. `sent` is a state rather than a dismissal - see the block offer. */
type Stage = 'form' | 'sending' | 'sent';

/**
 * The one report dialog, mounted once at the app shell.
 *
 * <p>Two things here are load-bearing rather than decoration:</p>
 *
 * <p><b>The evidence disclosure.</b> Reporting a message attaches a decrypted slice of the
 * conversation, because the server holds only ciphertext and a moderator would otherwise be
 * deciding on free text alone. Uploading part of someone's private conversation - even the
 * reporter's own - without saying so is not a thing to do quietly, so the dialog states what is
 * attached and lets the user read the exact payload before sending.</p>
 *
 * <p><b>The block offer after sending.</b> Blocking works immediately and needs nobody; reporting
 * is a queue. Offering only the queue leaves someone waiting on us while they are still being
 * messaged, so the block is offered at the moment it is most useful and is the visually primary
 * action of the confirmation.</p>
 */
@Component({
...
export class ReportDialogComponent {
    ...
    /**
     * The snapshot that will be sent, or null when there is none to send.
     *
     * <p>Recomputed rather than captured on open so that what the user reviews is what the request
     * carries. `capturedAt` is stamped at send time instead, since a dialog left open for a minute
     * should not claim the snapshot was taken a minute ago.</p>
     */
    ...
    /** `Report message` / `Report user` / … - one key per subject kind. */
