...
// `withKeyedIndex` hands its fetch's rows on and drops everything else the response carried, so the
// page cursor is parked here on the way past and read back by the view.
const RECIPE_CURSORS = new InjectionToken<WritableSignal<Record<string, string | null>>>(
    'MEAL_RECIPE_CURSORS',
    {providedIn: 'root', factory: () => signal<Record<string, string | null>>({})},
);
...
export const MealStore = signalStore(
    ...
    withKeyedIndex<Recipe, 'recipes', 'recipe'>({
        ...
        fetch: () => {
            ...
            const cursors = inject(RECIPE_CURSORS);
            return (channelId: string) =>
                api.listRecipes(channelId).pipe(
                    tap({
                        next: page => cursors.update(all => ({...all, [channelId]: page.nextCursor ?? null})),
                        error: () => cursors.update(all => ({...all, [channelId]: null})),
                    }),
                    map(page => page.items.map(wholeRecipe)),
                );
        },
    }),
    ...
    withMethods(
        (
            store,
            api = inject(MealApiService),
            lists = inject(ListStore),
            cursors = inject(RECIPE_CURSORS),
        ) => {
            const views = new Map<string, Signal<MealChannelState>>();

            const weekOf = (channelId: string): string =>
                store.weekByChannel()[channelId] ?? currentWeekStart();
            ...
            const setCursor = (channelId: string, cursor: string | null): void => {
                cursors.update(all => ({...all, [channelId]: cursor}));
            };
            ...
            return {
                ...
                loadMoreRecipes(channelId: string): void {
                    const cursor = cursors()[channelId] ?? null;
                    ...
                    api.listRecipes(channelId, undefined, cursor).subscribe({
                        next: page => {
                            if ((cursors()[channelId] ?? null) !== cursor) return;
                ...
                addRecipe(channelId: string, body: CreateRecipeDto): Observable<Recipe> {
                    return api
                        ...
                        .pipe(tap(recipe => upsertRecipe(channelId, recipe)));
                },
                ...
                generateShoppingList(
                    ...
                    return api.generateShoppingList(channelId, body).pipe(
                        tap(result => {
                            const listChannelId =
                                body.listChannelId ??
                                store.configByChannel()[channelId]?.shoppingListChannelId;
