...
export type EntitlementSubjectRef =
    | {kind: 'user'; id: 'me'}
...
export const EntitlementStore = signalStore(
    ...
    withMethods((
        ...
        const keyFor = (subject: EntitlementSubjectRef): string =>
            cacheKey(apiConfig.baseUrl(), profile.ownProfile()?.userId ?? 'anon', subject.kind, subject.id);
        ...
        return {
            ...
            ensureLoaded(subject: EntitlementSubjectRef): void {
                ...
                const request = subject.kind === 'guild'
                ...
                request.subscribe({
                    next: snapshot => {
                        ...
                        const discard = !answersFor(snapshot, subject)
            ...
            invalidate(subject: EntitlementSubjectRef): boolean {
                ...
                if (features) {
                    writeFeatures(key, {resolution: null, loading: false, requestId: features.requestId + 1});
                }
            ...
            uploadCeilingBytes(guildId: string | null): number | null {
                const own = numericCeiling(
                    valueOf(MY_ENTITLEMENTS, ENTITLEMENT_KEYS.userUploadMaxBytes));
    ...
    withComputed((store, apiConfig = inject(ApiConfigService), profile = inject(ProfileService)) => {
        const mine = computed(() => {
            const key = cacheKey(
                apiConfig.baseUrl(), profile.ownProfile()?.userId ?? 'anon', 'user', 'me');
    ...
    withHooks({
        onInit(store) {
            ...
            realtime.on('entitlements.Changed', (e: EntitlementsChangedDto) => {
                const subject: EntitlementSubjectRef = e.subjectKind === 'guild'
                    ? {kind: 'guild', id: e.subjectId}
                    : MY_ENTITLEMENTS;
