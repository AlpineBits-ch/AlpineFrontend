...
/**
 * `reconcile` in isolation, imported from the store rather than redefined here - so this suite
 * genuinely fails before the store exports it, instead of trivially passing against a copy that
...
describe('message cache reconciliation', () => {
    ...
    it('never widens to "everything the store holds for this conversation"', () => {
        // A live websocket arrival and an optimistic pending send would both show up if the
        // caller passed the full entity set instead of the cache-painted ids. Neither is a
        // cache-paint candidate, so neither may be dropped just because the server page - which
        // only ever describes what *it* returned - doesn't mention them. This function only ever
        // sees what it is given; the real guarantee lives in the caller passing the narrow set.
...
describe('MessageStore cache gap-fill - conversation', () => {
    ...
    it('ignores a cache read that resolves after the network page already landed', async () => {
        ...
        // Resolves synchronously, ahead of the cache promise chain, so `loadingMore` flips to
        // false before the cache read's `.then` callbacks ever run.
