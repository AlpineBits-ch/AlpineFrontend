...
/** A parsed mask plus whatever this build could not account for, so a save puts the residue back. */
...
export interface FlagGroup<K extends string> {
    ...
    labelKey: string;
    perms: K[];
    /** The `GuildFeatures` module gating this group. Absent means always shown. */
    feature?: string;
}
 ...
 * A JSON number cannot carry bit 63, so the server has to send names for masks that set it.
 */
