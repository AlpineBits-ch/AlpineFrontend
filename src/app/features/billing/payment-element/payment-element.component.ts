 ...
 *
 * <p>`message` is Stripe's own, which is written for cardholders and safe to render verbatim.
 * `key` is ours, for the cases Stripe cannot describe because it was never reached.</p>
 */
 ...
 *
 * <p>It is an iframe served by Stripe. That is not an implementation detail and not a shortcut: an
 * `<input>` collecting a PAN on our own origin moves this product from PCI SAQ-A to SAQ-D, which
 * means an annual audit and liability for a breach we would have designed in on purpose. The number
 * never enters our DOM, our memory or our logs, and nothing in this file can read it.</p>
 *
 * <p>Wrapped as a component because both surfaces that take a card - buying a plan, and adding one
 * from the payment methods list - mount the same element against a different kind of client secret,
 * and the mounting, the theming, the teardown and the "Stripe.js did not load" states are identical
 * in both. Writing them twice is how one of the two ends up leaking an element on close.</p>
 *
 * <p>The address is collected here too, on the same screen, because the server enables Stripe's
 * `automatic_tax` and that needs an address - and a second screen after the payment is a screen
 * people abandon.</p>
 */
@Component({
...
export class PaymentElementComponent {
    ...
    private cardHost = viewChild<ElementRef<HTMLElement>>('cardHost');
    private addressHost = viewChild<ElementRef<HTMLElement>>('addressHost');

    protected status = signal<'loading' | 'ready' | 'unavailable'>('loading');
    /**
     * Why there is no card field, when there is none.
     *
     * <p>`not_configured` and `load_failed` are two different sentences and only one of them has a
     * retry: an instance that sells nothing is a normal state, and a third-party script that did not
     * arrive is our side being broken while the customer is trying to pay us.</p>
     */
    protected reason = signal<StripeUnavailableReason | null>(null);
     ...
     *
     * <p>Loading Stripe.js is asynchronous and the secret can change underneath it - a customer who
     * closes a checkout and opens another one on a different plan. Without this, the first load
     * resolves second and mounts an element against a client secret for a subscription nobody is
     * buying any more.</p>
     */
     ...
     *
     * <p>`redirect: 'if_required'` keeps the customer on our screen for cards and wallets, which is
     * every method offered here. It is also what makes the result readable at all: the redirect
     * flavour navigates away and answers on a return URL instead.</p>
     *
     * <p><b>A null return means the card worked, and nothing more than that.</b> It is not an
     * activation and it is not an entitlement - only the webhook decides those - so the caller polls
     * afterwards rather than unlocking anything here.</p>
     *
