...
export class PlanPickerComponent {
    ...
    protected readonly catalogue = signal<BillingCatalogueDto | null>(null);
    ...
    protected readonly stripeReason = signal<StripeUnavailableReason | null>(null);

    protected readonly chosen = signal<BillingPlanDto | null>(null);
    protected readonly checkoutVisible = signal(false);
    ...
    protected readonly sellable = computed(() => this.catalogue()?.enabled === true && this.keyConfigured());

    /** The plans for this side of the product, in catalogue order - the server orders by price. */
    protected readonly columns = computed<PlanColumn[]>(() => {
