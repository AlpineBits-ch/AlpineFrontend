 ...
 *
 * `call-panel-full-view.spec.ts` only proves a *fresh* `CallPanelComponent` instance starts
 * `isFullView() === false`; it never touches `ConversationComponent`, `viewChild()`, or
 * `isCallFullView`, so it would still pass even if that fallback expression were deleted.
 *
 * Standing up a full `ConversationComponent` fixture to close that gap is out of proportion to the
 * one expression being tested - it pulls in `MessagingService`, three MLS services, `MessageStore`,
 * `ConversationStore`, `ConversationCallService`, and more, none of which this behaviour touches. So
 * this is a thin host component that reproduces the exact wiring
 * `ConversationComponent` uses - a `viewChild(CallPanelComponent)` behind an `@if`, and the same
 * `?? false` computed - and nothing else. It would fail if either the `viewChild` query or the `??
 * false` fallback were removed or changed.
 */
