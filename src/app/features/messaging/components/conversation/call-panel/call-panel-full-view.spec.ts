 ...
 *
 * `panelHeight` and `isFullView` are deliberately independent signals (see the doc comment on
 * `isFullView` in call-panel.component.ts): `toggleMaximize` never writes `panelHeight`, so a round
 * trip through full view always comes back out at the height the user last dragged to, never
 * DEFAULT_HEIGHT.
 */
