 ...
 *
 * <p>The channel and conversation views each own a send path, because each also owns an optimistic
 * bubble, a reply chip and a composer to clear. Sharing *into* somewhere - from the wiki, today -
 * has none of that: there is no open view to update and nothing to roll back. What it does still
 * need is the encryption rule, which is the part that must never be re-implemented casually, so
 * this is the same sequence `ChannelComponent.send` follows and nothing else.</p>
 */
