...
export class ForumSettingsComponent {
    ...
    protected savingTag = signal(false);
    protected reordering = signal(false);

    // ── Tag dialog ───────────────────────────────────────────────────────────
    protected showTagDialog = signal(false);
    protected draft = signal<TagDraft>(this.emptyDraft());
    /** The 409 "name already taken" is the one error users hit routinely - inline, not a toast. */
    protected nameTakenError = signal(false);
    protected showDeleteDialog = signal(false);
    protected tagToDelete = signal<ForumTag | null>(null);
    ...
    protected atTagCap = computed(() => this.tags().length >= this.maxTags);

    protected guildEmojiOptions = computed(() => [
    ...
    protected layoutOptions = computed(() => [
    ...
    protected archiveOptions = computed(() => [
    ...
    // ── Config ───────────────────────────────────────────────────────────────
    /**
     * Config edits save on change rather than behind a Save button: each field is a single
     * independent PATCH, so there is no partially-valid intermediate state to protect, and
     * a stray unsaved toggle here would silently not apply to the next post.
     */
    ...
    protected draftPreview = computed<ForumTag>(() => {
        const d = this.draft();
        return {
            id: d.id ?? 'preview',
    ...
    protected draftPreviewEmojiUrl = computed(() =>
    ...
    protected saveTag(): void {
        ...
        if (d.id) {
            // Empty string is how this API clears an emoji; null/undefined would mean
            // "leave unchanged" and the old emoji would survive the edit.
    ...
    private moveTag(from: number, to: number): void {
        ...
        // Positions are re-derived from the array index, matching how the server assigns
        // them, so the local list stays consistent with what the reorder call will produce.
