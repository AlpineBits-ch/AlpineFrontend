...
export interface InviteCandidateInput {
    ...
    /** Who the server says can see the channel; an empty list is read as "we could not find out" and offers nobody, matching {@link VoiceRingPickerComponent}'s collapse. */
...
/** Five: this is the shortcut, not the member list; a sixth name is another row to read before deciding nobody here is who you meant. */
export const INVITE_CANDIDATE_LIMIT = 5;

/**
 * Who to offer in the quick invite panel, in the order they are offered.
 * Friends first regardless of online status (the invitation waits in their conversation); everyone else must be online to earn a slot. Bots are dropped (can't join voice or read DMs).
 * Excludes yourself, whoever is already in the room, and anybody who can't see the channel; filtered here rather than left to the server, so probing this panel can't spend the ring budget to discover who can see a private channel.
 */
