/**
 * The lobby's "join and watch" is the one place a watch action is allowed to also join voice - the
 * user clicked an explicit join affordance, unlike the channel list's LIVE badge (Task 3, which only
 * opens the channel and arms a focus request). This exercises the full path: join, then arm.
 */
...
describe('VoiceChannelComponent join-and-watch', () => {
    it('joins the channel and arms a focus request for the streamer', async () => {
        ...
        // settle here either) would otherwise have to guess how many times it is safe to call it.
