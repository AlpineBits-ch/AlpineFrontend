...
/** `start` is the fork: make one, clone a template, or pull a Discord server across. The create path splits because the kind seeds the whole module set, and it's cheaper to answer up front than to unpick afterwards in settings. */
...
export class CreateGuildModalComponent {
    ...
    startDiscordImport(): void {
        ...
        this.discordImportService.startImport().subscribe({
            next: res => {
                ...
                // The user may have already cancelled/closed the modal while this request was in flight; don't slam it shut again or launch the OAuth browser in that case.
    ...
    submit(): void {
        ...
        // Owning a server is the most durable thing an account can do here, so it's the worst one to do without a key. Declining leaves the form exactly as typed.
    ...
    /** Typing a corrected id must retire the stale "not found" message immediately, rather than leaving it under the field until the next blur re-runs the lookup. */
    ...
    lookupTemplate(): void {
        ...
        this.guildTemplateService.get(id).subscribe({
            next: dto => {
                ...
                // The user may have already left template mode or changed the input while this request was in flight; don't resurrect abandoned template state in that case.
                if (this.isTemplateLookupStale(id)) return;
                this.template.set(dto);
            },
            error: err => {
                this.templateLoading.set(false);
                // Same stale-response guard as above: a late error for an abandoned lookup shouldn't surface a "not found" message or toast for state nobody cares about anymore.
                if (this.isTemplateLookupStale(id)) return;
                if (err?.status === 404) {
    ...
    createFromTemplate(): void {
        ...
        // Same gate as submit(). A server created from a template is no less a server, and this path reaches the same place by a different button.
        if (!this.socialGate.isSatisfied()) {
            void this.socialGate.require().then(allowed => {
            ...
            next: created => {
                this.guildService.getGuild(created.id).subscribe({
                    next: guild => {
                        ...
                        // The user may have already cancelled/left template mode while this request was in flight; don't slam the modal shut or navigate away in that case.
