...
/**
 * Pairs each removed line with the added line that replaced it. The unified diff emits a
 * replacement as a run of deletions then additions; buffering and zipping each run is what keeps
 * "was X" level with "now Y" across the gutter instead of rows apart.
 */
...
export interface RevisionDayGroup {
    ...
    key: string;
    date: Date;
    /** Left as a discriminator rather than a string: the label is translated, and a pure function has no business reaching for a translation service. */
...
export function groupRevisionsByDay(
    ...
    for (const revision of revisions) {
        ...
        if (!group) {
            ...
            byKey.set(key, group);
            // Input order is preserved rather than re-sorted: the server hands revisions back newest first, and re-deriving that ordering here would be a second opinion about it.
            groups.push(group);
        }
