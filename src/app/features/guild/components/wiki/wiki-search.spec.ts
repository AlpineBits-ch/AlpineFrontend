...
describe('searchWiki', () => {
    ...
    it('ranks an exact title above a prefix match', () => {
        const hits = searchWiki(
            [candidate({id: 'p', title: 'Setup Guide'}), candidate({id: 'e', title: 'Setup'})],
            'Setup',
        );
    ...
    it('ranks a title prefix above a mid-word substring', () => {
        const hits = searchWiki(
            [candidate({id: 'sub', title: 'Advanced Setup'}), candidate({id: 'pre', title: 'Setup Notes'})],
            'setup',
        );
    ...
    it('reports which field matched', () => {
        ...
        expect(searchWiki([candidate({id: 'b', title: 'X', tags: ['setup']})], 'setup')[0].matchedIn).toBe(
            'tag',
        );
    });
    ...
    it('narrows to a category and an author', () => {
        ...
        expect(
            searchWiki(items, 'setup', 25, {categoryId: 'c1'})
                .map(h => h.id)
                .sort(),
        ).toEqual(['a', 'c']);
...
describe('wikiPagePath', () => {
    ...
    it('walks categories then page ancestors, outermost first', () => {
        expect(wikiPagePath('leaf', pages, categories)).toEqual([
            'Handbook',
            'Operations',
            'Deploying',
            'Rollback',
    ...
    it('terminates on a category cycle', () => {
        ...
        expect(wikiPagePath('p', [{id: 'p', title: 'P', categoryId: 'x'}], cyclic)).toEqual(['Y', 'X', 'P']);
    });
});
