...
/**
 * The "what kind of page is this" step, shown once when a page is created.
 *
 * Deliberately a full picker rather than a dropdown on the editor: this is the first surface a
 * new page shows, and the difference between a wiki people fill in and one they abandon is
 * largely whether the first screen suggested a shape or handed over a blank rectangle.
 *
...
export class WikiTemplatePickerComponent {
    ...
    /**
     * On the document rather than the panel: the picker owns no focusable field to hang a keydown
     * on, and Escape has to work before the user has clicked anything.
     */
    ...
    protected choose(template: WikiTemplate): void {
        // `instant` at the moment of the click, not in a computed: this string is baked into the
        // saved page, so it has to be the language the author is writing in right now.
