...
/**
 * Assembles a template into the markdown the article inserts.
 *
 * Blocks are joined by a blank line rather than a single newline: markdown treats one newline
 * between two lines as a soft break, so `## Agenda` directly under a paragraph parses as part of
 * that paragraph and the heading never appears.
 */
...
/**
 * A pipe in translated prose ends the cell early and shifts every column after it; a newline ends
 * the table outright. Both are things a translator can introduce without ever seeing a table.
 */
