...
/**
 * How long a delete stays undoable. Long enough to notice the toast and react, short enough that
 * a user who walked away does not leave the wiki in a half-deleted state.
 */
...
export class WikiNavComponent implements OnDestroy {
    ...
    /**
     * Only one removal is undoable at a time. A queue would need per-toast dismissal, which
     * PrimeNG's MessageService cannot do - `clear(key)` clears them all - so a second delete
     * commits the first instead of silently stranding it without an Undo.
     */
    ...
    constructor() {
        ...
        // Recorded from the selection rather than from this component's own click handler:
        // pages are opened from search, backlinks, breadcrumbs and the home grid too, and a
        // "recently viewed" list that only knew about tree clicks would be wrong most of the time.
        effect(() => {
    ...
    protected onCategoryContextMenu(event: MouseEvent, category: WikiCategoryDto): void {
        ...
        // Every entry is gated, so an empty list means this member may do nothing here - showing
        // a blank menu would be worse than showing none.
    ...
    protected onPageContextMenu(event: MouseEvent, page: WikiPageSummaryDto): void {
        ...
        // Pinning shows the page in everyone's tree, so it is structure; an author who may edit
        // their own page is trusted with it too.
    ...
    /**
     * Commits on Enter and on blur, and is a no-op the second time - Enter clears `renamingId`,
     * which removes the input, which fires blur.
     */
    ...
    /**
     * Removes the page now and asks the server in a few seconds.
     *
     * A confirm dialog makes every delete cost a decision, including the 99% that were meant;
     * an undo makes only the mistaken ones cost anything, and it is the mistaken ones that a
     * modal never actually catches - people click through it.
     */
    protected deletePage(page: WikiPageSummaryDto): void {
        ...
        // Children are left alone: `buildPageTree` already treats a page whose parent is missing
        // as a root, so they surface one level up instead of vanishing with the parent.
    ...
    protected deleteCategory(category: WikiCategoryDto): void {
        ...
        this.state.updateWikiOptimistic(w => ({
            ...
            // Matches what the server does and what the old confirm dialog promised: the pages
            // survive, uncategorized.
    ...
    /**
     * Roving focus over whatever `[data-nav-item]` rows happen to be rendered.
     *
     * Driven off the DOM rather than an index into a model, because the rows come from five
     * separate sections (favourites, recent, pinned, categories, loose pages) and keeping a
     * flat index in sync with all of them would be a second tree to maintain.
     */
    ...
    /**
     * Runs an optimistic write and puts the tree back if the server refuses.
     *
     * The revert restores the snapshot instead of reloading: `getWiki` swallows its errors into an
     * empty wiki, so a reload that also failed would replace a wrong tree with no tree at all.
     */
    private withRevert(
    ...
    /**
     * Arms the undo window. `request` is only built when the delete actually commits, so an
     * undone delete never reaches the network.
     */
    private schedule(summary: string, snapshot: WikiDto, request: () => Observable<unknown>): void {
        ...
        const commit = () => {
            this.pending = null;
            // Resynced only once the server has agreed - reloading alongside the request would
            // race it and hand back the row that is being deleted.
