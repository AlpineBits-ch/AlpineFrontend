...
/**
 * Per-user nav preferences: favourited pages, recently viewed pages, folded categories.
 *
 * Local only, on purpose. None of this is anyone else's business, the wiki DTOs have nowhere to
 * put it, and a favourite that needed a round trip would be slower than the click that made it -
 * `isPinned` already covers the shared, everyone-sees-it case.
 *
 * Storage handling follows `wiki-drafts.service.ts`: every access is wrapped, because storage that
 * is disabled, full or corrupt must degrade to "no preferences" rather than break the tree.
 */
...
export class WikiNavPrefsService {
    ...
    /**
     * Points the service at a guild. Idempotent, because the caller is an effect that also runs
     * for unrelated signal changes; re-reading storage there would clobber unwritten state.
     */
    ...
    recordVisit(pageId: string): void {
        ...
        // A revisit of the page that is already at the front happens on every render pass of the
        // article; writing storage for it would be a write per change detection.
    ...
    private write(kind: PrefKind, value: readonly string[]): void {
        try {
            ...
            // Quota exceeded or storage disabled. Preferences degrade to session-only; the tree
