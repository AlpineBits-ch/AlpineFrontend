...
describe('WikiNavPrefsService', () => {
    ...
    // The nav records from the selected-page signal, which re-fires for reasons unrelated to
    // navigation; a re-record of the page already at the front must not reorder anything.
    ...
    // A second load for the same guild happens on every effect run; re-reading storage there
    // would throw away anything not yet written.
