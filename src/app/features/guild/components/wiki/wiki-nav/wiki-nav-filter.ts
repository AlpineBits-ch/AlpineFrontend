/**
 * In-place narrowing of the nav tree.
 *
 * Distinct from ⌘K: the palette navigates *away*, this one keeps you where you are and hides the
 * rows that do not matter. That difference is why it keeps ancestors of every match - a filtered
 * tree that drops the category a hit lives in stops being a tree and becomes a flat list, which is
 * what search already gives you.
 */
...
/**
 * Returns the ids still worth rendering, or `null` when there is no filter at all - the caller
 * then skips every set lookup instead of testing membership against "everything".
 */
export function narrowNav(
    ...
    // A category whose own name matches keeps everything under it: the user asked for that
    // section, not for the rows inside it that happen to repeat its name.
