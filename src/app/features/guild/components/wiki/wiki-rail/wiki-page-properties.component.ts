...
/**
 * `p-select` shows its placeholder for a null model, so "no category" needs a value of its own or
 * it renders as though nothing were chosen at all.
 */
const NONE = '';
...
/**
 * Page properties: tags, pinned, category, parent.
 *
 * These four have been in `WikiPageSummaryDto` since the feature shipped, and until now nothing in
 * the app could set any of them - the nav and the home page both render a Pinned section from a
 * flag no UI could raise, and search scores against tags nobody could add. The 2026-08-05 redesign
 * moved them here from the editor's meta row; this is that half of it.
 *
 * Editable while reading, not only while editing: they describe the page rather than being part of
 * it, and requiring a full edit session to pin something is what made them go unused.
 */
@Component({
...
export class WikiPagePropertiesComponent {
    ...
    /**
     * Fields written but not yet acknowledged.
     *
     * Held here rather than pushed into the open page so a control shows the new value the instant
     * it is clicked without the article's document being handed a new object mid-read. Presence of
     * the key is the test, not its value - `categoryId: null` is a real pending value.
     */
    ...
    /**
     * Where a page sits is the wiki's shape, so someone who manages the structure may move a page
     * they could not otherwise edit. Both branches deny while abilities are still loading, which
     * is the same fail-closed rule the rest of the wiki follows.
     */
    ...
    /**
     * Every page except this one and its own descendants.
     *
     * Reparenting a page onto something beneath it would cut that whole branch out of the tree and
     * leave it pointing in a circle, which the nav renders as nothing at all.
     */
    ...
    constructor() {
        // Keyed on the id, not the object: the wiki listing replaces the page object on every
        // refresh, and resetting on that would drop a write still in flight.
        effect(() => {
        ...
        // Retires an override once the page reports the value it was standing in for. Reading
        // `pending` through `update` rather than through the signal keeps this effect off its own
        // dependency list, and returning the same object when nothing matched keeps it quiet.
        effect(() => {
            const page = this.page();
            if (!page) return;
    ...
    protected addTag(raw: string): void {
        ...
        if (!tag) return;
        const tags = this.tags();
        // Case-insensitive, so "Setup" and "setup" cannot both end up on the same page and split
        // its search score in two.
    ...
    /**
     * Writes one property, showing it as done and putting it back if the server disagrees.
     *
     * The optimistic half touches the wiki listing - what the nav's Pinned section, the home page
     * and the search index all read - and deliberately not the open page. Handing the article a
     * new page object re-parses the document underneath the reader, and pinning a page is not a
     * reason to move somebody's scroll position.
     *
     * On success the override is left standing until the page itself reports the same value, so
     * the control cannot flicker back to the old one during the refresh that follows.
     */
    private apply(patch: PropertyPatch): void {
        ...
        // Our own write returns over the websocket a moment later; without this the page is
        // refetched and the article blinks through a spinner for a change already on screen.
        this.state.suppressPageRefreshOnce();

        this.wikiService.updatePage(this.guildId(), page.id, patch).subscribe({
            next: saved => {
                this.saving.set(false);
                // From the response rather than from the request: the server is free to trim or
                // reorder tags, and the listing should show what it actually stored.
 ...
 * into a page that is currently on screen.
 */
...
/**
 * Set comparison, not element-by-element: the server is free to return the tags in its own order,
 * and an override that refused to retire over that would outlive the write it stood in for.
 */
...
function subtreeIds(pages: readonly WikiPageSummaryDto[], rootId: string): Set<string> {
    ...
    let grew = true;
    // Repeated passes rather than a child index: a wiki is small, and a cycle already in the data
