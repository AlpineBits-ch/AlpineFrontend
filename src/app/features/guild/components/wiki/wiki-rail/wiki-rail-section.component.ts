...
/**
 * One collapsible block of the rail.
 *
 * The rail holds four sections now that page properties live in it, which is more than fits on a
 * short window. Collapse state is per section and persisted, because the sections a given person
 * ignores are the same ones every day - re-collapsing them on every page open is the annoying part.
 */
@Component({
    ...
    template: `
        <section class="mb-5">
            ...
            <!-- Left out of the document rather than hidden with a class, so a collapsed section
                 takes no space and its controls are not tab-reachable. -->
            @if (open()) {
                <div class="mt-1.5">
                    <ng-content/>
                </div>
