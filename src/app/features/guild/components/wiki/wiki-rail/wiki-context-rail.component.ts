...
/**
 * The right rail: page properties, table of contents, links in and out, attribution.
 *
...
export class WikiContextRailComponent implements OnDestroy {
    ...
    /**
     * An override for the scroll spy below. Left unbound the rail tracks the reader itself, which
     * is what it does today - nothing has ever fed this input.
     */
    readonly activeHeadingId = input<string | null>(null);
    /**
     * The body as it stands in the editor right now, so word count moves while typing rather than
     * reporting whatever was last published.
     */
    ...
    /**
     * Counted from the live editor body when there is one, so the figure is what the page will be
     * rather than what it was. `||` and not `??`: an editor that has not mounted yet reports an
     * empty string, and that is not a page with no words in it.
     */
    ...
    /**
     * Pages that name this one and never link it - the other half of "what points here", and the
     * half a wiki actually grows from.
     */
    ...
    constructor() {
        ...
        // A new document has new headings in new places, and the highlight must not keep pointing
        // at a section of the page before it. Deferred a tick: the headings arrive from the editor
        // before the DOM that holds them has been laid out.
        effect(() => {
            this.toc();
            setTimeout(() => this.syncActiveHeading());
        });

        // Capture phase on the document, because scroll does not bubble and the element that
        // scrolls belongs to the article, not to the rail.
    ...
    /**
     * Copies a link to one section. `wiki:<pageId>#<slug>` rather than a URL: it is the same href
     * the editor writes for a page link, so pasting it into a page produces a working link with no
     * new syntax to teach anybody.
     */
    ...
    /**
     * Turns the first plain mention on another page into a link to this one.
     *
     * The body is re-read from the server first rather than taken from the cache. The cache exists
     * to make backlinks cheap, not to be written back: a body that went stale between the warm and
     * this click would silently revert whoever edited it in between.
     */
    protected linkMention(row: MentionRow): void {
        ...
        this.wikiService.getPage(guildId, row.source.id).subscribe({
            next: fresh => {
                ...
                if (!linked) {
                    // The mention is gone from the current body. Refreshing the cache drops the
                    // row rather than leaving a button that does nothing.
    ...
    private readonly onScroll = (): void => {
        // One measurement per frame. Scroll fires far more often than that, and each pass reads
        // layout for every heading in the document.
        cancelAnimationFrame(this.scrollFrame);
        this.scrollFrame = requestAnimationFrame(() => this.syncActiveHeading());
    };

    /**
     * Which entry to highlight, from where the rendered headings currently sit.
     *
     * Matched by position rather than by id, so it works whether or not anything has stamped ids
     * onto the document - `buildToc` emits in document order, so the Nth element is the Nth entry.
     */
...
/**
 * The viewport position of the top of whatever scrolls the article.
 *
 * Found by walking up for an actual scrolling ancestor rather than by matching a class name: the
 * rail does not own that markup and must not break when its utility classes change.
 */
