 ...
 *
 * <p>There is no route to navigate to: the wiki is a `mainView`, so arriving at a page is three
 * moves (guild, then wiki, then page) against two services, the last of which cannot be made until
 * that guild's page listing has loaded. This holds the third move until it can be made, rather than
 * making every caller poll for a wiki it did not open.</p>
 */
...
export class WikiDeepLinkService {
    ...
    constructor() {
        effect(() => {
            ...
            // Somebody navigated away before the listing arrived. Dropping the target here is what
            // stops it firing later, when they next open that guild's wiki for their own reasons.
            ...
            // `guildId()` is set synchronously on initialize, so it is true before the fetch that
            // would make it useful. The loaded wiki's own guildId is the honest signal.
            ...
            untracked(() => {
                ...
                // A page created since this listing was fetched is absent from it but perfectly
                // readable. `openPage` only needs an id, so the full page stands in for a summary.
    ...
    /**
     * Whether a link into this guild's wiki could be followed at all - we are a member and the
     * module is on. Read by cards so "Open" is never offered where it would go nowhere.
     */
    ...
    open(guildId: string, pageId: string): boolean {
        ...
        // Both are no-ops when we are already there, and `selectServer` picking a channel first is
        // immediately overridden by `openWiki` - so a same-guild click never flashes a channel.
