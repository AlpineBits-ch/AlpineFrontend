...
/** Somewhere a page can be posted. Channels and conversations are one list on purpose - the choice
 *  a person is making is "who sees this", and that question does not care which of the two it is. */
...
/**
 * Posts a wiki page into a channel or a DM.
 *
 * <p>Self-contained: it owns its dialog, its picker and its send. The wiki mounts it and hands it a
 * page - see HANDOFF - because the page a share is about is whichever one is on screen, and that is
 * something only the wiki knows.</p>
 */
@Component({
...
export class WikiShareDialogComponent {
    ...
    /**
     * Text channels of the guild the page lives in, plus every DM.
     *
     * <p>Only the page's own guild: a page is readable by its members and nobody else, so offering
     * to post it into a different guild offers to post a link that everyone there will bounce off.
     * DMs are offered regardless - a link to somebody outside the guild is a decision a person is
     * allowed to make, and the card tells them plainly that they are not a member.</p>
     */
    ...
    constructor() {
        // The DM list is paged in on demand elsewhere; a share dialog opened before anyone visited
        // Direct Messages would otherwise show channels only.
    ...
    protected async share(): Promise<void> {
        ...
        try {
            ...
            this.state.set('idle');
            // Named separately: "this device cannot encrypt for that channel" is a state somebody
            // can act on - open it and let it catch up - and a generic failure is not.
