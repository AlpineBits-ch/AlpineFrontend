...
export class WikiGraphComponent {
    ...
    /**
     * Set when the canvas cannot be used at all. The list below is not a lesser fallback so much
     * as the same information without the physics, which is what a graph is for anyway.
     */
    ...
    /**
     * True once the user has panned or zoomed. A resize re-frames the graph, which is right until
     * somebody has deliberately chosen a viewport - dragging the nav divider must not keep
     * yanking them back out to the whole wiki.
     */
    private userAdjusted = false;
    /**
     * Theme colours resolved once per resize. Canvas takes strings, not classes, and reading
     * custom properties every frame would be a style recalc sixty times a second.
     */
    ...
    constructor() {
        // Same bargain the context rail makes: the warm is paid for by the feature that needs it,
        // when the user asks for it, rather than on every wiki load.
        effect(() => {
        ...
        effect(() => {
            ...
            const content = this.cache.content();
            // Positions carry over so the layout relaxes into the new data instead of restarting
            // from the seed ring every time a page is edited elsewhere in the guild.
            ...
            // The hovered node is a reference into the model that just got replaced; keeping it
            // would highlight a node that no longer exists.
            this.hovered.set(null);
            this.needsFit = this.needsFit || !this.userAdjusted;
            this.reheat();
        });

        // Re-runs when the canvas appears or is replaced - toggling to the list and back destroys
        // the element, and the observers have to follow it.
        effect(onCleanup => {
            ...
            if (!context) {
                // No 2d context at all (a locked-down embedded webview, a lost GPU). The list is
                // still the whole graph, minus the fun.
            ...
            // A simulation nobody can see is pure battery. rAF already stops for a hidden tab;
            // this covers the case the tab is visible and the graph simply is not.
    ...
    protected resetView(): void {
        ...
        // Fitted here and not by reheating the simulation: "fit to view" is an answer to a click,
        // and waiting two seconds of physics for it is not an answer.
    ...
    protected onPointerDown(event: PointerEvent): void {
        ...
        if (index !== null) {
            ...
            // Pinned for the duration so the springs pull the neighbourhood towards the pointer
            // rather than pulling the grabbed node out of it.
    ...
    protected onPointerMove(event: PointerEvent): void {
        ...
        // Only touch the signal on a real change: this fires on every mouse move, and setting a
        // signal to the value it already holds still schedules change detection.
    ...
    protected onWheel(event: WheelEvent): void {
        ...
        // Zoom about the pointer rather than the centre, so the thing being examined stays under
        // the cursor instead of sliding off the edge.
    ...
    /**
     * Hit tolerance is specified in screen pixels and converted, so a node stays as easy to hit
     * when the graph is zoomed out as when it fills the canvas.
     */
    ...
    /**
     * One frame: step, cool, draw, and schedule the next only while there is still movement.
     * A settled graph costs nothing until something asks for a repaint.
     */
    private readonly loop = (): void => {
        ...
        if (this.alpha > ALPHA_MIN) {
            ...
            // Fitting mid-layout would chase the nodes around; fitting once they stop is a single
            // decisive move to the right framing.
            this.fit(model);
        }
    ...
    /**
     * Canvas takes colour strings, so the theme tokens have to be resolved rather than applied as
     * classes. Read here rather than hard-coded so `ThemeService`, which overrides these custom
     * properties at runtime, still reaches the graph.
     */
