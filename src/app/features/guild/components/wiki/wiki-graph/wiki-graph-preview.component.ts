...
/**
 * The still frame of the graph that sits on the home page.
 *
 * <p>Static SVG, not a second live canvas: this exists to make the graph look like somewhere worth
 * going, and a simulation running behind a card the user is not looking at is exactly the cost the
 * real view goes to some trouble to avoid.</p>
 *
 * <p>It deliberately does <b>not</b> warm the content cache. Edges need every page body, which is
 * a full-content request, and paying for that on every wiki open - for a decoration - is the
 * decision {@link WikiContentCacheService} documents itself as not making. So the preview draws
 * whatever links are already cached: a bare constellation on a cold start, the real shape once
 * anything else in the wiki has warmed it.</p>
 */
@Component({
