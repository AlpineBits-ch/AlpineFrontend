 ...
 *
 * <p><b>Why a hand-written simulation.</b> Every force-graph library on npm is tens of kilobytes
 * and brings its own renderer, its own event model and its own opinions about the DOM. What is
 * actually needed here is three forces and an integrator, which is the eighty lines below. A wiki
 * with three hundred pages is well inside what a naive O(n²) repulsion pass does in a frame, so
 * the quadtree the libraries exist to provide buys nothing at this size.</p>
 *
 * <p>Layout is deterministic: initial positions come from a hash of the page id, never from
 * `Math.random()`. Re-opening the graph therefore lands on the same picture, and the static
 * preview on the home page agrees with the full view instead of showing a different wiki.</p>
 */
...
export function buildGraph(
    ...
    // Links first: the node cap wants to keep the most connected pages, which cannot be known
    ...
    for (const [sourceId, targets] of linksBySource) {
        ...
        for (const targetId of targets) {
            ...
            // Undirected: A links to B and B links back is one line, not two drawn on top of
            // each other at double opacity.
...
export function stepSimulation(model: GraphModel, alpha: number): void {
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (let j = i + 1; j < count; j++) {
            ...
            if (d2 < 0.01) {
                // Two nodes at the identical seed would divide by zero and stay stuck together
                // forever. Nudge them apart along a fixed axis so the result stays deterministic.
                dx = 0.1;
                dy = 0;
                d2 = 0.01;
            }
 ...
 * Category colour.
 *
 * <p>Generated on a golden-angle hue rotation rather than picked from a hard-coded list: the
 * number of categories is not known ahead of time, and any fixed list either runs out or repeats.
 * The rotation gives adjacent indices maximally distant hues, and the fixed saturation and
 * lightness keep every category legible on the dark surface. Uncategorised pages deliberately fall
 * back to the theme's own accent, read from the live custom property so runtime theming applies.
 */
...
/**
 * A stable starting point per page id.
 *
 * Placed on a ring rather than in a filled disc: starting every node near the centre makes the
 * first dozen frames an explosion, which is the one moment the user is definitely watching.
 */
