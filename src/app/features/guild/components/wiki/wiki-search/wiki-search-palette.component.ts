 ...
 *
 * A query that matches nothing used to be a dead end. It now offers to create the page, which is
 * usually what the user was going to do next anyway.
 */
@Component({
...
export class WikiSearchPaletteComponent implements AfterViewInit {
    ...
    /**
     * Results, plus the two things that are not results: recent queries when there is nothing to
     * search yet, and the create action when nothing matched what was typed.
     */
    protected readonly rows = computed<PaletteRow[]>(() => {
        ...
        // Offered only when nothing already carries that exact title - otherwise the action
        // invites a duplicate of the page sitting one row above it.
    ...
    constructor() {
        ...
        effect(() => {
            if (this.open()) {
                ...
                // Filters left armed from last time would silently hide results the next search
                // should have found.
                this.clearFilters();
        ...
        // The author filter is a list of names, not of user ids, so the profiles have to be in
        // hand before the dropdown is opened rather than after.
        effect(() => {
    ...
    protected choose(page: WikiPageSummaryDto): void {
        // Recorded here rather than on keystroke: a query is only worth remembering once it has
        // proven it leads somewhere.
    ...
    /**
     * Creates the page and opens it. Done here rather than handed to the editor as a prefilled
     * title because the editor has no such input, and a palette that dropped the user into an
     * empty "Untitled" would have thrown away the one thing they had already typed.
     */
