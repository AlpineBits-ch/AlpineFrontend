...
/**
 * The last few queries that actually led somewhere.
 *
 * Recorded on selection rather than on keystroke: "de", "dep", "depl" are not six searches, they
 * are one, and a list of prefixes is worse than an empty list. Local only, same reasoning as
 * `wiki-nav-prefs.service.ts` - and the same defensive storage handling as
 * `wiki-drafts.service.ts`, because a full or disabled localStorage must not break ⌘K.
 */
...
export class WikiSearchRecentsService {
    ...
    private write(value: readonly string[]): void {
        try {
            ...
            // Storage disabled or full. History degrades to session-only rather than throwing
            // inside the palette's keydown handler.
        }
    }
