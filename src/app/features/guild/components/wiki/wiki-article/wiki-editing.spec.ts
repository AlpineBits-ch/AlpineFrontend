...
/**
 * The same run the Tab handler inserts. Non-breaking, because a literal tab at the start of a line
 * reparses out of markdown as an indented code block.
 */
const INDENT = ' '.repeat(4);

/**
 * The editing behaviours that are only true if the whole extension list agrees.
 *
 * Every one of these was reported as the editor ignoring a keystroke, and every one of them comes
 * from two extensions each assuming the other bound the key. A unit test on either one in isolation
 * would have passed - so these build the real list.
 */
describe('wiki editor keys', () => {
    ...
    describe('Enter in a list', () => {
        ...
        it('breaks out of a list that has content after it', () => {
            ...
            // A paragraph between the list and the heading, with the caret in it - not back up in
            // "only item", and not merged into the heading below.
            const kinds: string[] = [];
            editor.state.doc.forEach(node => kinds.push(node.type.name));
            // Sliced because the trailing-node extension keeps its own empty paragraph at the end
            // of the document, which is not what this test is about.
        ...
        it('breaks out of an indented bullet to the level above', () => {
            ...
            // Out of the nested list, but still in the outer one - two Enters is one level, not
            // an exit from the whole structure.
    ...
    describe('checkbox from a bullet', () => {
        it('turns "- [ ] " into a task item', () => {
            // The state the user is actually in: they typed `- `, the input rule already fired,
            // and they are standing in an empty bullet item about to type the brackets.
...
/**
 * Copying a page out of the wiki. The complaint was three blank lines between every paragraph,
 * which turned a pasted page into a column of whitespace.
 */
...
/**
 * Switching between the rich surface and the markdown one is serialise-then-reparse, so anything
 * that does not survive the round trip is silently deleted from somebody's page. This is the whole
 * check behind "switching modes must never lose data".
 */
