...
/** Strings the block extensions need, in a plain object rather than looked up via TranslateService: node views and plugins run outside Angular's injector and change detection. The English defaults here are the fallback; wikiBlockLabels() overlays translated values when available. */
...
/** Resolves the labels once at editor construction; instant() returns the key itself on a miss, so a miss falls back to the English default instead of showing the raw key. */
