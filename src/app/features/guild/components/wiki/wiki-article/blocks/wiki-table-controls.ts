...
/**
 * Everything a table needs after it has been inserted.
 *
 * `/table` used to drop a 3x3 grid and then leave you with it: no row could be added, no column
 * removed, and the only way out was source mode. This is that missing half.
 *
 * It is a decoration rather than an Angular component so that it needs nothing from the article
 * template - the controls are part of the editor's own DOM, positioned by the document rather than
 * by a floating-element library, and they disappear on their own the moment the caret leaves the
 * table or the page drops back to read mode.
 */
...
function buildControls(editor: Editor, labels: WikiBlockLabels): HTMLElement {
    ...
    for (const action of ACTIONS) {
        ...
        // On mousedown, and suppressed: by the time a click fires the caret has already left the
        // cell, and every one of these commands is defined relative to where the caret is.
...
/**
 * Column resizing, but only while editing.
 *
 * Tiptap's own `Table.configure({resizable: true})` decides once, at editor construction, by
 * reading `editor.isEditable` - and this editor is constructed in read mode and only later told it
 * is editable, so that switch is permanently off here. Registering the ProseMirror plugin directly
 * and gating its props on the live value is the same feature without that ordering trap.
 */
function editableColumnResizing(editor: Editor): Plugin {
    const base = columnResizing({
        ...
        // The table node view comes from the Table extension; a second one here would be ignored
        // anyway, since direct editor props win over plugin-provided views.
        View: null,
    });
