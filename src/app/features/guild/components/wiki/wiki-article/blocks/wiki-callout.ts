...
/**
 * GitHub's alert syntax, which is what this serialises to. Chosen over a `:::info` fence because
 * it is the one callout dialect that renders as *something* everywhere else: a plain markdown
 * viewer shows a blockquote whose first line names the severity, rather than three stray colons.
 */
...
/** `> [!TIP] ` typed at the head of a block. The `> ` half is optional because Blockquote's own
 *  input rule usually swallows it first, leaving only the marker to match. */
...
export const WikiCallout = Node.create<WikiCalloutOptions>({
    ...
    /**
     * The icon is a node view rather than part of `renderHTML` because it doubles as the control
     * that cycles the variant - a decorative span in the render spec could not carry a click
     * handler, and a separate toolbar button for something this local is worse than clicking it.
     */
    addNodeView() {
        ...
        return ({node, editor, getPos}) => {
            ...
            marker.addEventListener('mousedown', event => {
                ...
                // caret out of the callout body and the transaction below would fight it.
    ...
    /**
     * Its own token type rather than a second handler on `blockquote`.
     *
     * Sharing the token name did parse correctly, but the serialiser resolves a *node's* renderer
     * through the same token registry - so an ordinary blockquote found this extension first and
     * came back out of the editor as `> [!NOTE]`. A dedicated token keeps the two apart. Extension
     * tokenizers run ahead of every built-in block rule, so this still gets first look at the text
     * and hands anything that is not an alert straight back to marked's own blockquote rule.
     */
    markdownTokenizer: {
        name: 'wikiCallout',
        level: 'block',
        ...
        return [
            new InputRule({
                ...
                handler: ({state, range, match, chain}) => {
                    ...
                    // Blockquote's `> ` rule has almost always already fired by this point, so the
                    // caret sits inside a quote. Retyping that quote as a callout keeps one box
                    // instead of nesting a callout inside a blockquote.
