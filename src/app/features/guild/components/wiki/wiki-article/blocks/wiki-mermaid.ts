 ...
 *
 * The import is dynamic because mermaid is the largest thing in the dependency tree by a wide
 * margin and almost no page contains a diagram - pulling it into the initial chunk would make the
 * app slower to start for everyone so that a minority of pages can draw a flowchart. It is a real
 * dependency rather than a CDN script: this ships inside a Tauri shell that has to work with no
 * network at all.
 */
...
/**
 * Mermaid cannot read CSS custom properties, so the palette is sampled once and handed over as
 * theme variables. Read at load time rather than baked in, so a runtime brand colour change from
 * ThemeService is picked up the first time a diagram is drawn.
 */
...
function load(): Promise<MermaidModule> {
    loading ??= import('mermaid').then(module => {
        ...
        mermaid.initialize({
            startOnLoad: false,
            // Wiki content is written by other guild members, so labels are never trusted to
            // contain markup - `strict` is the level that actually sanitises them.
