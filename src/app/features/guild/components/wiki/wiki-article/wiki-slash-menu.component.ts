...
/**
 * Something the menu cannot do with an editor chain alone, because the state it needs lives on the
 * article: the hidden file input, and the `[[` trigger that has to survive the menu closing.
 */
...
export interface SlashItem {
    ...
    labelKey: string;
    /** One line under the label. Not filtered on - descriptions match far too loosely. */
    ...
    keywords: string;
    /**
     * The whole command, for items the editor can do by itself. Absent on AI and host items -
     * those carry a discriminator below instead and are dispatched by the article, which is the
     * only place that holds the AI service, the page body and the file input.
     */
...
/**
 * Block insertion, opened by typing `/`.
 *
 * Replaces the block half of the old toolbar. A filtered list you type into scales past what a
 * row of icons can hold, and it costs nothing while you are not using it. Grouped with a
 * description per row rather than a bare icon list, because the menu is now long enough that
 * scanning it is the slow part - and because "Expand" and "Improve writing" are not self-evident
 * from a label the way "Quote" is.
 */
@Component({
                            ...
                            <button #itemEl (click)="choose(entry.item)"
                                ...
                                <span class="flex h-7 w-7 shrink-0 items-center justify-center
                                    ...
                                    <!-- Colour folded into the one binding: two competing text-*
                                         ...
                                         order, which Tailwind decides, not by which is written
                                         last here. -->
...
export class WikiSlashMenuComponent {
     ...
     * `filtered` list below is what the arrow keys index, and the grouping is a view over it.
     */
    ...
    /**
     * The rendered shape: sections in declaration order, each row carrying its index in the flat
     * list so the highlight and the arrow keys agree without a second lookup.
     */
    ...
    constructor() {
        // Typing another character can shorten the list out from under the highlight, and Enter on
        // an index that no longer exists silently does nothing.
        effect(() => {
            const length = this.filtered().length;
            if (untracked(this.activeIndex) >= length) this.activeIndex.set(0);
        });

        // The list is taller than its box once every group is showing, so arrowing past the fold
        // has to bring the highlight with it or the selection disappears off-screen.
        effect(() => {
