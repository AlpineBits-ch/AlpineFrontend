...
export function wikiExtensions(
    ...
    return [
        StarterKit.configure({
            ...
            // allowlist, a sanitisation control that must not be left to chance.
            link: false,
            underline: false,
        }),
        Underline,
        ...
        // resizable stays off: it is decided once at construction from an isEditable that is still false in read mode, so it would never turn on; WikiTableControls registers the resize plugin itself, gated on the live value.
