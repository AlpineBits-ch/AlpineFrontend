...
/**
 * A mention href, e.g. `user:0198…`. Same shape as `wiki:` page links and for the same reason: an
 * ordinary Link mark round-trips through the markdown serializer for free, so a mention survives
 * save and reload with no custom node and no custom serializer.
 *
 * `user` has to be in the Link extension's protocol allowlist or the mark is dropped at parse
 * time - see HANDOFF.
 */
 ...
 * The `@` member picker.
 *
 * Server-side search like the chat composer's mention overlay, rather than a member list held in
 * memory: a guild roster can run to thousands and the wiki has no reason to hold one.
 */
@Component({
...
export class WikiMentionMenuComponent {
    ...
    /**
     * Debounced, and nothing is fetched while the menu is shut - `@` in prose is common enough
     * that an eager request per keystroke would be a steady trickle of roster calls from anyone
    ...
    private readonly members = toSignal(
        toObservable(this.request).pipe(
            ...
            switchMap(request => {
                ...
                // A failed lookup shows "no members match", which is true enough for a picker and
                // better than an unhandled error while someone is typing.
