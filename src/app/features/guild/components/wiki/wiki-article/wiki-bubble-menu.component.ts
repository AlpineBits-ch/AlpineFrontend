...
/**
 * English names for the languages we ship locales for.
 *
 * The picker shows the endonym, because that is what a person recognises; the model is told the
 * English name, because "Français" as a target language is one more thing for it to guess at.
 * Anything added to SUPPORTED_LANGUAGES without an entry here still works - it just sends its own
 * label.
 */
...
/**
 * Inline formatting and the AI actions that only make sense on a selection.
 *
 ...
 *
 * The AI half sits behind one sparkles button past a divider rather than as six more icons. It
 * emits an intent and nothing more - the article routes it to the inline bar, which owns the
 * document while the rewrite streams in and can put the selection back if it is rejected.
 */
@Component({
        ...
        @if (visible()) {
            <div [style.left.px]="position().left" [style.top.px]="position().top"
                ...
                <div class="flex items-center gap-0.5 rounded-lg border border-border bg-card
                    ...
                    <!-- The seam between "change these characters" and "ask a model to rewrite
                         them". Two different kinds of act; they should not read as one row. -->
                ...
                @if (aiMenuOpen()) {
                    <!-- mousedown is swallowed so the editor never blurs: the selection this
                         menu is about is the selection the action needs a moment later. -->
...
export class WikiBubbleMenuComponent {
    ...
    protected submenuItems(): SubmenuOption[] {
        ...
        return SUPPORTED_LANGUAGES.map(language => ({
            id: language.code,
            // Endonyms are already in their own language; translating them would be a round trip
