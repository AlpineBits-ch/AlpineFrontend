...
/**
 * Inline autocomplete: a grey continuation of the sentence you stopped in the middle of.
 *
 * This is the feature with the widest gap between magic and awful, and the difference is almost
 * entirely about *when it does not fire*. It is off unless the user turned it on, it costs the
 * user's own API credit on every pause, and a suggestion that appears while you are still typing
 * is worse than none at all. Hence the deliberately narrow gate in {@link shouldSuggest}, the
 * hard abort on every keystroke, and the single-line result.
 *
 * Tab accepts. Anything else dismisses. Esc dismisses and stays quiet until you type again -
 * "no thanks" has to mean more than "not this millisecond".
 */
...
/**
 * A `/`, `[[`, `:` or `@` run means one of the trigger menus is open or about to be. Matched
 * locally rather than by importing the trigger plugin: that file belongs to another stream, and
 * all this needs to know is "something else owns this keystroke".
 */
...
export function wikiGhostTextPlugin(options: WikiGhostTextOptions): Plugin {
    ...
    const request = (view: EditorView) => {
        ...
        void options
            ...
            .then(result => {
                // Three ways this answer can already be stale: a newer request, a moved caret, or
                // an edit that landed while it was in flight. All three make it wrong to show.
            ...
            .catch(() => {
                // A failed completion is silent on purpose. Nothing was asked for out loud, so
                // there is nothing to apologise for - and an error toast on a typing pause would
                // be the most irritating error in the app.
            });
    };
    ...
    return new Plugin<GhostState | null>({
        ...
        props: {
            decorations: state => {
                ...
                return DecorationSet.create(state.doc, [
                    Decoration.widget(ghost.pos, () => ghostElement(ghost.text), {
                        side: 1,
                        // Without this the widget counts as a selection boundary and the caret
                        // jumps to the far side of the suggestion.
            ...
            handleKeyDown: (view, event) => {
                // Unconditional: a request in flight is a request for a caret that is about to
                // move, whether or not a suggestion is currently on screen.
                ...
                // Every other key dismisses, and does not consume: the keystroke was meant for
                // the document.
                clear(view);
                return false;
            },
        ...
        view: () => ({
            update: (view, prevState) => {
                ...
                // Only typing starts the clock. Clicking around a document is navigation, and
                // suggesting text at every place the caret lands is how this feature becomes
    ...
    function shouldSuggest(view: EditorView): boolean {
        ...
        // Code is the one place a plausible-looking continuation is actively dangerous: it reads
        // as something that was checked, and nothing here checked it.
        ...
        // Mid-word and mid-sentence completions fight the user for the same characters. Only a
        // caret with nothing after it in its own block is a caret with room to continue.
 ...
 * One line only.
 *
 * A multi-paragraph ghost is unreadable as an overlay, and `insertText` would put its newlines
 * inside a single paragraph as literal characters. The provider contract already says this is one
 * short shot rather than a stream, for the same reason: half a suggestion is worse than none.
 */
...
function ghostElement(text: string): HTMLElement {
    ...
    // Styled inline rather than from the article's stylesheet: this plugin has to be droppable
    // into the editor without a matching CSS change landing in a file it does not own.
