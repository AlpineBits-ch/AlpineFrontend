...
/** The article as the AI dialog reaches it: draftWithAi is a public method the wiki view calls on a ?., so anything unresolved on the way to the inline bar fails by doing nothing at all. */
describe('WikiArticleComponent AI entry points', () => {
    ...
    beforeEach(async () => {
        ...
        await TestBed.configureTestingModule({
            ...
            providers: [
                ...
                // WikiStateService -> GuildWebsocketService -> MlsService -> MlsEngine. Nothing on the AI path encrypts anything, so inert fakes for every port are enough.
                provideFakePlatform(),
                // The article reaches WikiService -> ApiConfigService -> OAuthService. Nothing on the AI path makes a request, so the base URL is all that is needed.
                {provide: ApiConfigService, useValue: {baseUrl: () => 'https://api.test.example'}},
                {provide: OAuthService, useValue: {getAccessToken: () => 'tok'}},
                // Reached through the websocket service and initialises against the Tauri OS plugin, which is not there under jsdom.
                {provide: NotificationService, useValue: {createNotification: async () => undefined}},
                // The article reports a failed markdown conversion through ToastService, which needs PrimeNG's MessageService. No toast is raised on the AI path.
                MessageService,
                // The real service reads its bindings from localStorage on construction, which this environment does not provide. Nothing on the AI path presses a shortcut.
    ...
    /** The wiki hands the article a fresh page object on every summary refresh (anyone's edit to any page, a pin, a tag); cancelling the AI bar on that discarded a generation streaming into a page nobody had navigated away from. */
