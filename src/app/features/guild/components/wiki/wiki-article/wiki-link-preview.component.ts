...
/**
 * What a `wiki:` link points at, without going there.
 *
 * A wiki is a graph, and following an edge to find out it was not the one you wanted costs a page
 * load plus a trip back. This answers the question in place.
 *
 * The snippet is whatever the content cache already happens to hold - pages fill it as they are
 * opened, and the full warm is one request over the whole wiki. Hovering a link is not a good
 * enough reason to spend that, so a cold link shows its metadata and no body, rather than stalling
 * behind a fetch nobody asked for.
 */
@Component({
