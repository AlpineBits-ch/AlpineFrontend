 ...
 * The `:` emoji picker.
 *
 * Backed by `EmojiDataService`, the same shortcode index the chat composer autocompletes against,
 * so `:tada` means the same thing in a page as it does in a message and the wiki adds no second
 * emoji table to the bundle. The plugin will not open this before two query characters, which is
 * what keeps "Note: a thing" from popping a picker mid-sentence.
 */
@Component({
...
export class WikiEmojiMenuComponent {
    ...
    /**
     * Resolved on first open rather than injected up front. `EmojiDataService` pulls the emoji
     * dataset in its constructor, and most wiki sessions never type a colon - injecting it as a
     * field would download it for all of them.
     */
    ...
    constructor() {
        ...
        // The result set changes shape on every keystroke, and Enter on a row that no longer
        // exists does nothing at all, which reads as a broken menu.
        effect(() => {
    ...
    handleKey(key: string): boolean {
        ...
        if (key === 'Enter' || key === 'Tab') {
            ...
            // Not consumed when there is nothing to pick: Enter has to keep splitting the
            // paragraph while a two-letter colon run happens to match no emoji.
