...
/**
 * "Ana edited Rules, 2h ago".
 *
 * The wiki's front page used to list the same page titles the nav already shows, which gives
 * somebody who opens it no reason to have opened it. This is the part that changes, so the page is
 * worth checking rather than worth skipping.
 */
@Component({
...
export class WikiActivityFeedComponent {
    ...
    /**
     * 30 s cadence, shared with everything else in the app that renders a time. A per-row interval
     * would be one timer per entry for text that changes once a minute.
     */
    ...
    constructor() {
        ...
        // `app-avatar` resolves the face on its own, but the name beside it is read from the
        // cache synchronously, so something has to ask for it. Resolving here rather than per row
        // means one pass over the visible entries instead of one effect per row - and
        // `resolveByUserId` is already a no-op for anyone cached, which after the first render is
        // nearly everyone.
        effect(() => {
