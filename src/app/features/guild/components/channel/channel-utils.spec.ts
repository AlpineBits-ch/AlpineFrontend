...
describe('forumParentOf', () => {
    ...
    // A post can arrive before its parent is in the cached channel list; that must be null, not a crash, or the whole main view fails to render.
...
describe('mayPostCleartext', () => {
    ...
    it('refuses cleartext when we are in the group but have no generation recorded', () => {
        // A torn registry write or a wipe mid-session lands here; sending plaintext into an encrypted channel is never the safe interpretation of an inconsistent local state.
    ...
    it('refuses cleartext whenever a generation is held, whatever the resolved state says', () => {
        // Holding a generation is direct evidence the channel is encrypted for this device, and it outranks a resolved state that may simply be stale.
    ...
    it('refuses cleartext in any channel this device has ever encrypted', () => {
        // Generation cleared, state resolved to plain, and still refused: the floor is the only input here the server cannot move.
        expect(mayPostCleartext(null, 'plain', 2)).toBe(false);
    });

    it('treats generation zero as a floor, not as absent', () => {
        // The first generation of a context is 0, and `0` is falsy: a truthiness check here would exempt exactly the contexts that have only had one encrypted era.
        expect(mayPostCleartext(null, 'plain', 0)).toBe(false);
    });

    it('still allows cleartext where no floor was ever set', () => {
        // The floor must not turn every plaintext channel into a refusal: a veto that fires everywhere would need to be removed.
