...
// Handed to both masks: each parser keeps only the names its own space defines, so one list reaches whichever of the two actually owns the permission.
...
describe('PantryChannelComponent', () => {
    ...
    describe('the module gate', () => {
        ...
        /** "Owner turned it off" and "plan doesn't cover it" are the same absence from what is effective, with opposite remedies, so they are two states here. */
    ...
    describe('the loop master switch', () => {
        ...
        it('is off with none, whatever the items\' thresholds say', () => {
            ...
            // The row is still honestly low; it just cannot be appended anywhere, which is what the banner and the struck-through threshold say.
    ...
    describe('the item editor and the null-vs-zero threshold', () => {
        ...
        // The four regressions below all share one failure mode: the request succeeds, the toast says saved, and the server changed nothing, because a nullable field sent as `null` reads as "leave alone" on the wire. Each asserts the whole body, not one key, so a stray `lowThreshold: null` alongside the flag would fail too.
