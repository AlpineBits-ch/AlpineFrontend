...
/**
 * The join flow for a member who hasn't accepted yet: a non-dismissable, full-screen
 * step-through covering the welcome, the rules and each prompt in turn.
 *
 * Everything is answered locally and submitted in one `accept` call at the end - the
 * API has no partial-progress concept, so a per-step save would leave a member who
 * quits halfway both pending and partly granted.
 */
@Component({
...
export class OnboardingGateComponent {
    ...
    /**
     * An option's emoji is either a unicode character or a guild emoji id, with no
     * discriminator - the step component tells them apart by looking for a hit here.
     */
    ...
    protected answers = signal<Record<string, string[]>>({});
    protected showRequiredError = signal(false);
    ...
    protected submitting = computed(() => this.state.isAccepting());
    ...
    protected prompts = computed(() =>
        [...(this.status()?.prompts ?? [])].sort((a, b) => a.position - b.position));

    /**
     * The welcome step is always present so the flow opens on the server's name rather
     * than a wall of rules; every other step is conditional on there being content.
     */
    ...
    protected currentStep = computed<Step | undefined>(() => this.steps()[this.stepIndex()]);
    protected isFirstStep = computed(() => this.stepIndex() === 0);
    protected isLastStep = computed(() => this.stepIndex() >= this.steps().length - 1);

    /** Only prompt steps count toward the progress bar - the framing steps aren't work. */
    protected promptProgress = computed(() => {
    ...
    constructor() {
        effect(() => {
            ...
            untracked(() => {
                ...
                // Switching servers mid-flow must not carry the previous guild's answers
                // or leave the wizard parked on a step that no longer exists.
    ...
    protected finish(): void {
        ...
        this.state.accept(this.guildId(), responses, () => {
            // Roles and channel visibility both change on accept, so the cached guild is
            // stale the instant this resolves - re-read it rather than patching guesses in.
    ...
    protected leaveServer(): void {
        // The wizard is deliberately non-dismissable so a pending member can't bypass the
        // rules - but they still need a way out that isn't "reload the app". Switching the
        // workspace off 'server' is what unmounts it (see main-page.component.html), so
        // this is a navigation away, not a bypass.
