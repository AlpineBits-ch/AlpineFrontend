...
/**
 * One prompt's options, rendered the way the admin declared: MultipleChoice as
 * selectable cards, Dropdown as a real select. Shared by the join wizard and the
 * Channels & Roles screen so both stay identical as the shapes evolve.
 *
 * Purely controlled - it owns no selection state, which is what lets the wizard hold
 * every answer until the single accept call at the end.
 */
@Component({
...
export class OnboardingPromptStepComponent {
    prompt = input.required<OnboardingPrompt>();
    ...
    /**
     * Derived here rather than re-exporting the enum as a field: that couples the view to
     * module-evaluation order, which is fragile under the test runner's module graph, and
     * a boolean is what the template actually wants.
     */
    protected isDropdown = computed(() => this.prompt().type === OnboardingPromptType.Dropdown);
    ...
    protected singleValue = computed<string | null>(() => this.selected()[0] ?? null);
     ...
     * discriminator - a hit in the url map is the only reliable way to tell them apart.
     */
    ...
    protected toggle(option: OnboardingPromptOption): void {
        ...
        if (this.prompt().singleSelect) {
            // Re-picking the current answer clears it, unless the prompt is required -
            // there, clearing would only produce a validation error the user can't act on.
