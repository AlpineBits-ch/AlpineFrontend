...
export class ModulesSettingsComponent {
    ...
    /**
     * Set while the *kind* is saving. That one genuinely locks the page: a kind change can
     * re-seed the entire module set, so a toggle flipped alongside it would race the re-seed.
     * A module save, by contrast, only locks its own row - see {@link savingModules}.
     */
    protected savingKind = signal(false);
    /** The modules with a save in flight, by flag name, so three of them can be flipped in a row. */
    protected savingModules = signal<ReadonlySet<string>>(new Set<string>());
    ...
    protected focusedKind = signal<GuildKind | null>(null);

    /**
     * What the user asked for, per module, until the server confirms or refuses it.
     *
     * The switches read this layered over `features()` rather than binding straight to the
     * guild: with a one-way binding to server truth, a *failed* save changed nothing, so
     * Angular never wrote the old value back and the switch stayed sitting where the user
     * dragged it - reading "on" right next to the error toast that says it isn't.
     */
    private optimistic = signal<ReadonlyMap<string, boolean>>(new Map<string, boolean>());
    ...
    protected displayedFeatures = computed<ReadonlySet<string>>(() => {
    ...
    protected tabbableKind = computed(() => this.focusedKind() ?? this.kind());
     ...
     *
     * Deliberately off `features()` and not `displayedFeatures()`: a row must not vanish
     * from under the switch that is still saving, only once the server has agreed.
     */
    ...
    protected toggle(module: string, enabled: boolean): void {
        ...
        // Built from what is *shown*, not from what the server last confirmed: with two toggles
        // in flight at once, a payload off server truth would silently undo the other one.
        ...
        this.save({features: serializeGuildFeatures(next)}, () => {
            ...
            // Dropping the entry hands the switch back to server truth either way: after a
            // success that is the value we just asked for, after a failure it is the revert.
    ...
    /**
     * Roving focus over the kind options, which is what `role="radiogroup"` promises and
     * five separate tab stops did not deliver.
     *
     * Focus moves, selection does not follow it: picking a kind opens a confirmation dialog,
     * so arrowing down the list would fire one dialog per keypress. Space and Enter commit,
     * which the underlying buttons already do for free.
     */
