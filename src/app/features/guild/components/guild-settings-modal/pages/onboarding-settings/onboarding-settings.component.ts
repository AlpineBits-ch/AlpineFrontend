...
export class OnboardingSettingsComponent implements OnInit {
    ...
    protected readonly mode = signal<OnboardingMode>(OnboardingMode.Default);
    protected readonly rulesText = signal('');
    protected readonly defaultChannelIds = signal<string[]>([]);
    protected readonly prompts = signal<OnboardingPrompt[]>([]);

    // ── Prompt editor ────────────────────────────────────────────────────────
    protected readonly showPromptEditor = signal(false);
    protected readonly editingPrompt = signal<OnboardingPrompt | null>(null);
    private readonly editingIndex = signal<number | null>(null);

    protected readonly showRemovePrompt = signal(false);
    protected readonly promptPendingRemoval = signal<OnboardingPrompt | null>(null);
    private readonly removeIndex = signal<number | null>(null);

    // ── Welcome screen ───────────────────────────────────────────────────────
    protected readonly welcomeEnabled = signal(false);
    protected readonly welcomeDescription = signal('');
    protected readonly welcomeChannels = signal<WelcomeChannel[]>([]);

    // ── Pending members ──────────────────────────────────────────────────────
    protected readonly pendingMembers = signal<PendingMember[]>([]);
    ...
    protected readonly configDirty = computed(() => this.configSnapshot() !== this.configBaseline());
    protected readonly welcomeDirty = computed(() => this.welcomeSnapshot() !== this.welcomeBaseline());
    protected readonly dirty = computed(() => this.configDirty() || this.welcomeDirty());
    ...
    protected readonly availableWelcomeChannels = computed(() => {
    ...
    /** Removal is local until Save, and even then, deleting a prompt does not take back roles or channels it already granted; only a member deselecting an option revokes. */
    ...
    protected save(): void {
        ...
        // A full-document PUT: anything absent from this payload is deleted server-side, which is why the prompt ids read back from the GET are round-tripped verbatim.
        ...
        this.safety.updateOnboardingConfig(this.guild().id, config).subscribe({
            next: saved => {
                ...
                // Adopt the response: newly created prompts and options come back with their generated ids, and dropping them would re-create them next save.
    ...
    protected saveWelcomeScreen(): void {
        ...
        this.safety.updateWelcomeScreen(this.guild().id, screen).subscribe({
            next: () => {
                ...
                // Baseline on the renumbered list that was actually sent, so the half goes clean even when a removal left the local positions one step behind.
