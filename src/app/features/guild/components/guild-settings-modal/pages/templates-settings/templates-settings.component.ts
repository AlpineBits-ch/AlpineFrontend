...
export class TemplatesSettingsComponent implements OnInit {
    ...
    /** Every template made since this page was opened; the API has no way to list templates back (create, get-by-id and use are the only routes), and an id that isn't copied is an id that is gone, so holding them here is the only recall there is until a listing route exists. */
    readonly created = signal<CreatedTemplateDto[]>([]);
