...
/**
 * Bills: what the house owes, and the standing arrangements that produce it.
 *
 * <p><b>A bill is an obligation before it is an expense</b>, and this panel exists because the
 * ledger next door could only ever say the second thing. "Rent is due Friday and you owe 850" is not
 * a line of history and must not be drawn as one - nothing has been spent, nobody is owed, and there
 * is nothing to settle. Posting is what moves it across.</p>
 *
 * <p>Two rules the editor enforces rather than discovers. `autoPost` needs a fixed amount, because
 * there would be nothing to post otherwise. And a variable bill cannot be posted without somebody
 * naming a figure, which is why the post dialog asks for one rather than offering a bare button.</p>
 */
@Component({
...
export class BillsPanelComponent {
    ...
    canAddExpense = input.required<boolean>();
    ownUserId = input.required<string | null>();
    memberOptions = input.required<Option[]>();
    ...
    protected state = computed(() => this.bills.stateFor(this.channelId()));

    /**
     * What is still owed, soonest first.
     *
     * <p>Posted and skipped periods are deliberately absent. A posted one is an expense now and is
     * on the ledger's own list; a skipped one is a decision the house already took. Keeping either
     * here would turn the board answering "what is coming" into a second, worse expense history.</p>
     */
    protected upcoming = computed<BillRow[]>(() =>
    ...
    protected overdueCount = computed(() => this.upcoming().filter(r => r.state === 'overdue').length);
    protected needsAmountCount = computed(() => this.upcoming().filter(r => r.state === 'needs-amount').length);

    protected scheduleRows = computed<ScheduleRow[]>(() => this.state().schedules.map(schedule => ({
        schedule,
    ...
    protected showSchedules = signal(false);
    ...
    protected formHasFixedAmount = signal(true);
    ...
    protected formUnit = signal<RecurrenceUnit>(RecurrenceUnit.Month);
    protected formInterval = signal(1);
    protected formAnchorAt = signal('');
    protected formLeadDays = signal(3);
    protected formAutoPost = signal(false);
    protected formPaused = signal(false);

    protected formAmountMinor = computed(() =>
    ...
    /**
     * The one combination the server refuses outright, checked here so the switch can explain
     * itself rather than collect a `400` from a Save the user has already pressed.
     */
    protected autoPostBlocked = computed(() =>
        recurringExpenseError({
    ...
    protected unitOptions = computed<Option[]>(() =>
    ...
    protected categoryOptions = computed<Option[]>(() => this.categories.map(category => ({
        value: category,
        label: this.translate.instant(expenseCategoryLabelKey(category)),
    })));

    // ── Post dialog ──────────────────────────────────────────────────────────
    protected showPostDialog = signal(false);
    protected postingBill = signal<BillOccurrence | null>(null);
    protected postAmount = signal('');
    protected postAmountMinor = computed(() => parseMinor(this.postAmount(), this.currency()));

    /**
     * Whether the figure on screen would be accepted.
     *
     * <p>A blank box is legal for a bill that already carries an amount - it means "post what the
     * schedule says" - and is not for one whose amount varies, where there is nothing to fall back
     * on.</p>
     */
    protected postValid = computed(() => {
    ...
    // ── Skip dialog ──────────────────────────────────────────────────────────
    protected showSkipDialog = signal(false);
    protected skippingBill = signal<BillOccurrence | null>(null);
    protected skipReason = signal('');
    ...
    /**
     * `ManageLedger`, or `AddExpenses` when the caller is the payer.
     *
     * <p>The payer clause is the important half: confirming what you yourself just paid the landlord
     * is the ordinary case, and needing a moderator for it would mean the one person who knows the
     * figure cannot enter it. The payer lives on the *schedule*, not on the occurrence, so this has
     * to resolve it.</p>
     */
    ...
    /**
     * Turning the fixed amount off also turns auto-post off.
     *
     * <p>The pair is illegal server-side, and leaving the switch on so the form can then refuse to
     * save is a worse answer than resolving it here: nobody wants an automatic post of an amount
     * they have just said they do not know.</p>
     */
    ...
    protected submitSchedule(): void {
        ...
        const request$ = editingId
            ? this.bills.editSchedule(channelId, editingId, {
                description,
                // The pair that expresses "this bill's amount varies now". A bare null would read
                // as "leave the amount alone", which is the opposite instruction.
            ...
            : this.bills.addSchedule(channelId, {
                ...
                payerUserId: this.formPayerUserId() ?? undefined,
                // Equal over an empty `shares`, which the server reads as everyone in the guild -
                // the same default the expense form takes, and the only one that stays correct when
                // a flatmate moves in halfway through a tenancy.
