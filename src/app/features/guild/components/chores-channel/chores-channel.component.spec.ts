...
function setup(
    ...
    // Stubbed rather than real: the store reaches an HTTP service and the realtime hub, and what this component asks it is one question with four possible answers.
    const entitlements = {
...
describe('ChoresChannelComponent module gate', () => {
    it('says the house does not run a rota rather than accusing the user, when the module is off', () => {
        // §13.2: a 403 from a household endpoint usually means the module is off, and the owner is not exempt; reading `features` first is what lets this say the right sentence.
    ...
    /** Both states are the same absence from `effective` with opposite remedies (a toggle here vs. one that can't be flipped at any permission level), so they must not share a sentence. */
...
describe('ChoresChannelComponent chore editor', () => {
    it('opens a new chore already on the Flatmates rota', () => {
        ...
        // The seeded Flatmates role *is* the rotation pool, so a house that has one starts a new chore with the pool already chosen.
        expect(component.assignmentError()).toBeNull();
    });

    it('will not save a chore with neither a rotation role nor a fixed assignee', () => {
        // A house with no Flatmates role (renamed, or a guild that only switched the module on) has nothing to default to, so the picker asks rather than guessing at whichever role sorts first.
