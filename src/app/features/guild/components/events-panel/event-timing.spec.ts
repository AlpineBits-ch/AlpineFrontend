...
// Built from local-time components, not a UTC string: `dayBucket` compares local calendar days, so
// a UTC-pinned fixture would bucket differently depending on the machine's timezone.
...
describe('phaseOf', () => {
    ...
    it('keeps an event with no end time live for the grace window, then drops it', () => {
        // "Game Night at 8" with no end is the common case. Before this window existed it was live
        // for zero seconds and appeared under Past the minute it began.
    ...
    it('treats a blank endsAt exactly like an absent one', () => {
        // `new Date('')` is NaN and NaN compares false both ways, so an unguarded comparison drops
        // the event out of every list at once.
...
describe('dayBucket', () => {
    ...
    it('buckets just after midnight as tomorrow, not today', () => {
        // 40 minutes away by the clock, but a different calendar day - which is what a reader means
        // by "tomorrow".
