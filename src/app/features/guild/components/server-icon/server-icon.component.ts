...
export class ServerIconComponent implements OnDestroy {
    readonly serverData = input.required<ServerData>();
    ...
    protected onImgError(): void {
        ...
        if (this.retryCount < delays.length) {
            ...
            const timer = setTimeout(() => {
                ...
                // Probe silently: only swap back to the image if the retry actually loads, so the fallback letter doesn't flicker on each retry attempt.
