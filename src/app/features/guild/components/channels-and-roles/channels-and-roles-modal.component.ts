...
/**
 * The post-join "Channels & Roles" screen: every prompt the guild defines, including
 * the ones hidden from the join flow, with the member's current picks editable.
 *
 * Save is a full replace across all prompts - a prompt omitted from the payload is read
 * as "nothing selected" and its grants are revoked, so the whole set always goes up
 * together, never a delta.
 */
@Component({
...
export class ChannelsAndRolesModalComponent {
    ...
    protected sortedPrompts = computed(() => [...this.prompts()].sort((a, b) => a.position - b.position));
    ...
    private originalAnswers = signal<Record<string, string[]>>({});

    constructor() {
        effect(() => {
            const open = this.visible();
            const guildId = this.guildId();
            // Re-read on every open: another device (or the join wizard) may have changed
            // these picks since the last time this dialog was mounted.
    ...
    protected save(): void {
        ...
        // Required prompts must still have an answer afterwards - catching it here beats
        // a 400 that names an id the user never sees.
        ...
        // Every prompt is sent, including the empty ones - that's how a deselection is
        // expressed, and omitting them would be read the same way anyway.
