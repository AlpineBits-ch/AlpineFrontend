...
function setup() {
    ...
    // The constructor eagerly loads the guild picker's options; irrelevant to the
    // error-mapping behaviour under test here, so just drain it.
    ctrl.expectOne(r => r.url === `${BASE}/api/v1/guild/guilds`).flush([]);

    // Bypass the Select-driven guild/channel pickers and go straight to a submittable
    // state, since these specs are only about what confirm() does with the HTTP response.
