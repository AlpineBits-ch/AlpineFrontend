...
@Component({
  ...
  imports: [Button, PermissionOverrideEditorComponent],
...
export class CategoryPermissionsComponent implements OnInit {
  ...
  saveRole(row: RoleOverride): void {
    ...
    this.guildService.upsertCategoryPermission(this.category().id, {
      ...
      error: () => this.roleOverrides.update(list => list.map(r => r.role.id === row.role.id ? {...r, saving: false} : r)),
    });
  }
  ...
  deleteRole(row: RoleOverride): void {
    ...
    this.guildService.deleteCategoryPermission(this.category().id, row.perm.id).subscribe({
      next: () => {
        this.roleOverrides.update(list =>
          list.map(r => r.role.id === row.role.id ? {...r, perm: null, override: {allow: 0n, deny: 0n}, dirty: false} : r)
        );
      },
    });
  }
  ...
  saveMember(row: MemberOverride): void {
    ...
    this.guildService.upsertCategoryPermission(this.category().id, {
      ...
      error: () => this.memberOverrides.update(list => list.map(r => r.member.id === row.member.id ? {...r, saving: false} : r)),
    });
  }
  ...
  deleteMember(row: MemberOverride): void {
    ...
    this.guildService.deleteCategoryPermission(this.category().id, row.perm.id).subscribe({
      next: () => {
        this.memberOverrides.update(list =>
          list.map(r => r.member.id === row.member.id ? {...r, perm: null, override: {allow: 0n, deny: 0n}, dirty: false} : r)
        );
      },
    });
  }
