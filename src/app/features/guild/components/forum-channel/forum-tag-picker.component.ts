...
/**
 * Multi-select chip picker over a forum's tags, capped at the server's 5-per-post limit.
 *
 * Moderated tags are filtered out for users who can't apply them rather than shown and
 * rejected - the backend 403s the whole create/update request when one slips through, so
 * offering the chip at all would mean losing the user's entire post body to a permission
 * error.
 */
@Component({
...
export class ForumTagPickerComponent {
    ...
    protected atCap = computed(() => this.selected().length >= this.maxTags);
    ...
    protected toggle(tagId: string): void {
        ...
        // Keep the set in the forum's own tag order so what the user sees here matches
        // the order the server returns applied tags in on the post card.
