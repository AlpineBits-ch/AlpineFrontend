...
/**
 * One tag pill. Used in the filter bar, on post cards and in the picker, so it takes
 * its interactive state from inputs rather than owning any.
 *
 * A tag colour of #000000 is the server's default for "no colour chosen", not a real
 * black - those render in the neutral surface style instead of an unreadable black pill.
 */
@Component({
...
export class ForumTagChipComponent {
    ...
    emojiUrl = input<string | null>(null);
    ...
    protected textColor = computed(() => {
        ...
        // Lift the raw tag colour toward white so a dark user-picked colour stays legible
        // on the dark surface; selected pills lift further to read as "on".
