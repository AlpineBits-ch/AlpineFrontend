...
/**
 * Guards on the phone-number copy, because the wording <i>is</i> the feature here.
 *
 * <p>Three things the server guarantees, none of which the UI may contradict, and all three are
 * easy to break with a well-meaning edit rather than with a bug:</p>
 *
 * <ol>
 *   <li><b>Nothing verified the number.</b> There is no `isVerified` field in any response and
 *       there never will be - a backend test pins the DTO's property count so nobody adds one,
 *       because the only value such a flag could carry is false. So no string here may call a
 *       number verified, confirmed or checked.</li>
 *   <li><b>A member with no number and a member who opted out are indistinguishable</b>, by design:
 *       somebody who has not consented is never named on the bus to Identity at all. So no string
 *       may say "has not shared her number", which asserts that she has one. The <i>caller's own</i>
 *       account is the one exception, and only because the client reads it off `/users/self`.</li>
 *   <li><b>The number is plaintext; the handles are ciphertext.</b> They arrive in one response as
 *       separate lists precisely so a client can keep them apart, and somebody deciding whether to
 *       share a number deserves to be told which kind it is.</li>
 * </ol>
 *
 * <p>Covers three screens now: the per-house sharing card in the ledger (`PAY.PHONE.*`), the pay
 * sheet that shows somebody else's number (`PAY.SHEET.PHONE_*`), and the account field where a
 * number is actually entered (`ACCOUNT.PHONE.*`). The third one is the write side, so it carries a
 * fourth rule of its own: <b>the `+` is explained rather than guessed at</b>. The server refuses a
 * leading `00` instead of rewriting it, because that rewrite is right in most of the world and wrong
 * in enough of it to store a stranger's number, and the field has to say so.</p>
 *
 * <p>Asserted against the shipped locale file rather than against constants, because the string a
 * user reads is the one in that file.</p>
 */
