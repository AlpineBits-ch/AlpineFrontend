 ...
 *
 * <p>The directory is the only thing faked, because that is the wire. A stubbed `isAvailable()`
 * would assert the stub rather than which capability gates which half.</p>
 */
 ...
 *
 * <p>The QR-bill, the provider links and the copyable account numbers all come out of a sealed blob,
 * so a browser cannot produce any of them and says so. The TWINT assist is a phone number the server
 * already holds in plaintext and a copy button - nothing cryptographic anywhere in it, and the one
 * route on this sheet a browser can complete end to end. A single `isAvailable()` over the whole
 * template hid it there, which is the same defect as the editor's and in the same direction: a
 * working control hidden rather than a dead one offered.</p>
 */
describe('PaySheetComponent capability split', () => {
     ...
     *
     * <p>Asserted through `FakeSecureStore.reads`, which records the entry <i>names</i>, so this
     * fails loudly if the web path ever starts fetching a key under a different name.</p>
     */
