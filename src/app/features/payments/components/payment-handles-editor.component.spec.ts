 ...
 * set for `host`.
 *
 * <p>Deliberately not a stubbed service: what these tests are about is which capability gates which
 * half, and a fake `isAvailable()` would assert the stub rather than the wiring.</p>
 */
 ...
 *
 * <p>The sealed handles are opened and written with this device's Ed25519 seed, which a browser has
 * no OS-protected home for, so they are hidden there and that is correct. The phone-number opt-in is
 * a consent flag plus a number the server already holds in plaintext; nothing about it depends on a
 * keychain. They were gated together, so the browser hid a control that works - the inverse of the
 * defect the capability pass was written to fix, and the more expensive direction, because a hidden
 * working control looks like a missing feature rather than like a broken one.</p>
 */
