 ...
 *
 * <p><b>The validation here is the only validation there is.</b> The server stores ciphertext and
 * checks sizes; it cannot read an IBAN, so it cannot tell a good one from a typo. A mistyped IBAN
 * does not bounce back from anywhere - it either fails silently a week later or reaches whoever
 * owns the account it turned into. The mod-97 check runs before the value can be saved, and the
 * message names what is wrong rather than saying "invalid".</p>
 *
 * <p><b>The address is asked for last and only when it is needed.</b> A Swiss QR-bill requires a
 * structured creditor address - name, post code, town, country - and that is a home address, which
 * is a real step up in what we are holding about somebody. It is collected once an IBAN is actually
 * added and never demanded from a person who only ever uses PayPal.</p>
 *
 * <p>Saving is a two-step on purpose: work out who the seal would reach, show the devices that
 * cannot prove whose they are, and only then write. See
 * {@link RecipientTrustReviewComponent} for why that question is not optional.</p>
 */
@Component({
        ...
        <div class="flex flex-col gap-3">
            @if (available()) {
                <div>
                    ...
                    <!--
                      The guarantee, stated exactly and no further. Encryption here defends against
                      the database being disclosed. It does not defend against a malicious server,
                      because the server serves the key directory and could substitute a key.
                    -->
            ...
            @if (available()) {
                ...
                <!--
                  Never worded as a confirmation of anything. What is shared is a claim about where
                  to send money, and the app has verified none of it.
                -->
                <p class="m-0 text-xs text-text-muted">
...
export class PaymentHandlesEditorComponent {
     ...
     *
     * <p>The two writes are unrelated: sharing a number is a consent flag on the guild member row,
     * and sealing handles is a blob. Nothing about one should block the other.</p>
     */
     ...
     *
     * <p>Read from `UserService.self`, which `GET /users/self` already fills and which already
     * projects `phoneNumber` - no endpoint was added for this. It is the caller's <i>own</i> record,
     * so no privacy line is crossed by knowing it, and it is what turns "if your account has a
     * number, this house can now see it" into a statement rather than a hedge.</p>
     */
     ...
     *
     * <p>Two steps rather than one because the plan is what the trust review is <i>about</i>: the
     * user has to see which devices cannot prove whose they are before anything is written, and a
     * one-press save would have shown them that after the fact.</p>
     */
     ...
     *
     * <p>The number is not editable here. It is one value on the account, and offering to edit it
     * from inside one household would suggest a number per household - which is neither what is
     * stored nor what would happen.</p>
     */
     ...
     *
     * <p>The plan is recomputed from the server every time rather than cached, because the roster
     * and the attestation state can both have moved since the page opened - and a plan computed
     * against a stale directory is a plan that seals to a device nobody was shown.</p>
     */
     ...
     *
     * <p>The switch renders from the server's echoed state rather than from what was clicked, so a
     * write that failed leaves it showing the truth. Getting that backwards on this particular
     * control would tell somebody their number was private when it was not.</p>
     */
     ...
     *
     * <p>Otherwise a user could review the recipient list, edit their IBAN, and press Seal against
     * a plan that was computed for a different payload - which is a confirmation that was never
     * actually given for what got written.</p>
     */
