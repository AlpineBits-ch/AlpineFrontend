 ...
 *
 * <p><b>Why this is a screen at all.</b> The recipients endpoint publishes each device's
 * attestation state precisely so a client can decline one that cannot prove who it is, and the
 * backend's own note says nothing forces a client to look. Reading the flags and doing nothing with
 * them would be worse than not having them: it would let us describe the feature as end-to-end
 * encrypted while sealing to whatever key the directory happened to return. So a flagged device is
 * never sealed to silently, and never dropped silently either - a person who thinks they shared
 * with both of Ben's phones and reached one is worse off than a person who was asked a question.</p>
 *
 * <p><b>The fingerprint is the review</b>, exactly as it is on the MLS join-request screen, and the
 * string is the same one: this device's key is its MLS signature key, so what is printed here is
 * byte-identical to what that device shows for itself. Comparing it is a thing users have already
 * been taught to do, and it is the only check here that does not go through the server.</p>
 */
@Component({
