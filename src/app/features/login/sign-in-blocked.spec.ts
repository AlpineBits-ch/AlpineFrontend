...
describe('signInBlocked', () => {
    it('reads the marker off an unparseable plain-text body', () => {
        ...
        const err = {status: 403, error: {error: new SyntaxError('bad json'), text: 'User is not allowed to sign in'}};
        expect(signInBlocked(err)).toEqual({reference: null});
    });

    it('reads it off a bare string body', () => {
        expect(signInBlocked({status: 403, error: 'User is not allowed to sign in'})).toEqual({reference: null});
    });
    ...
    it('reads it off problem-details style fields', () => {
        expect(signInBlocked({status: 403, error: {detail: 'User is not allowed to sign in'}})).toEqual({reference: null});
        expect(signInBlocked({status: 403, error: {message: 'user is not allowed to sign in.'}})).toEqual({reference: null});
    });
    ...
    it('picks up a reference code once the server sends one', () => {
        const err = {status: 403, error: {detail: 'User is not allowed to sign in', reference: 'vnt-2h4k9x7p'}};
