 ...
 *
 * <p>Component names and descriptions are the server's words, in the server's display order. The
 * only thing composed here is the state label next to the dot, which is a fixed five-value table
 * with a neutral fallback - an unknown state gets a grey dot and the raw key, never an alarming
 * colour and never a crash.</p>
 *
 * <p>The 90-day strip that the real status page draws is deliberately not reimplemented; the button
 * goes there instead.</p>
 */
@Component({
