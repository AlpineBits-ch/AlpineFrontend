 ...
 *
 * <ol>
 *   <li><b>Push to talk.</b> The web publisher's gate reads the VAD and never looks at `inputMode`, so
 *       choosing push-to-talk in a browser moved a radio and changed nothing. Disabled with a reason,
 *       not removed: it is the mode people arrive expecting.</li>
 *   <li><b>The sensitivity slider and its live meter.</b> Web only, and shown even when the stored mode
 *       is push-to-talk - an account that last used the desktop app arrives with that mode stored, and
 *       the browser gates on speech regardless, so the one control that decides whether they can be
 *       heard must be reachable.</li>
 *   <li><b>The output picker.</b> Gated on `outputSupport().selectable`, never on the speaker list being
 *       empty. An empty list is what both "no speakers" and "Firefox will not say" look like, and only
 *       one of those makes the picker a no-op.</li>
 *   <li><b>Withheld device names.</b> Actionable, so it earns a line: starting the mic test grants the
 *       permission that reveals them.</li>
 * </ol>
 */
