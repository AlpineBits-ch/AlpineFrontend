 ...
 *
 * <p>The switch was already disabled where nothing can bind the local presence channel - that is the
 * `canDetect` gate this component's own comment documents - but it was disabled with <b>nothing said</b>,
 * which reads as a bug rather than as a host limitation. The per-game list above it had carried its
 * reason since it was written; this row had not.</p>
 *
 * <p>Both directions are asserted, because a reason line that appeared on the desktop too would be a
 * plain falsehood and a browser-only test would not notice.</p>
 */
