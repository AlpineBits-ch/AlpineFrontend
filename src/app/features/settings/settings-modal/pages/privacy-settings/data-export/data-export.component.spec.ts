 ...
 *
 * <p><b>Both paths are dead in a browser, and that is why this is a gate rather than a fallback.</b> The
 * native streaming path needs the desktop shell; the webview path is blocked outright, because the
 * endpoint answers `302` to a signed Google Cloud Storage URL that serves no
 * `Access-Control-Allow-Origin`. So the button did not merely sometimes fail - it failed every time,
 * reporting "could not download that export" for an artifact sitting there intact.</p>
 *
 * <p>Requesting an export is deliberately left alone on both hosts: it is a server-side job, and a user
 * can perfectly well start it here and collect it from the desktop app.</p>
 */
