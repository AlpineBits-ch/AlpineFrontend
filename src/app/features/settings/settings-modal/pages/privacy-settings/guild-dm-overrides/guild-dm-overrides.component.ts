 ...
 *
 * <p>Only shown when the global policy is `FriendsAndServerMembers`, because that is the only
 * setting these can narrow. Under `Friends` or `Nobody` the global rule already refuses server
 * members, and under `Everyone` there is nothing to qualify - rendering the list anyway would
 * offer a row of switches that change nothing.</p>
 */
@Component({
