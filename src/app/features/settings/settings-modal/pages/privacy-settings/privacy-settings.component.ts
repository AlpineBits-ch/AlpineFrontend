 ...
 *
 * <p>Rows are declared as data and rendered by a single piece of markup each, so adding a field to
 * the spec means adding a line here rather than another copy of a flex row. Writes go straight
 * through as partial PATCHes on change - there is no Save button, because a page of twenty
 * independent switches has no coherent "save everything" moment and a half-applied save is worse
 * than none.</p>
 *
 * <p>When {@link PrivacySettingsService.status} is not `ready` every control is disabled. Showing
 * writable switches over an unknown record would reproduce precisely the problem this page exists
 * to fix.</p>
 */
@Component({
