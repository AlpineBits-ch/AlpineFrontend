 ...
 *
 * <p><b>The server was never repaired.</b> After a restore the keychain holds the backup's signing
 * key while the device row still advertises the `identityPublicKey` of whatever key this device had
 * before, and the device has no key packages at all - so a device that had just successfully
 * restored every key it needed still could not be added to anything. Only `resetKeyPackages` ran.</p>
 *
 * <p><b>The reinstall case could never work.</b> A fresh install mints a new device id, so
 * `import_backup` - which restores the ratchet state only when the ids match - correctly refused,
 * and every message from before the reinstall stayed unreadable on what is physically the same
 * machine. Rust's own comment assumes the caller "adopts the backup's device id first"; nothing
 * did, and there was no way to.</p>
 */
