 ...
 *
 * <p>Most of the behaviour lives in {@link AccountSwitchService} and is tested there. What is only
 * true here is the wiring: the service's call check defaults to "no call", which is the safe answer
 * for the check and the wrong one to leave in place - a switch reloads the process, and a reload
 * during a call drops it with no warning. This component is the only place that owns both.</p>
 */
