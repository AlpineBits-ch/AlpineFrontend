 ...
 *
 * <p>Lives in the profile popover, which is the bottom-left avatar menu - where the equivalent sits
 * in every client that has one, and the only place in this app that already means "you, and what
 * you can do about being you".</p>
 *
 * <p><b>One account is live at a time.</b> Switching re-enters the app as the other account rather
 * than running two sessions; see {@link AccountSwitchService} for why. The rows for the accounts
 * that are not live are therefore not "connected elsewhere" - they are stored sessions, and the
 * copy avoids implying otherwise.</p>
 */
@Component({
