        ...
        return this.mlsService.createGroup(groupIdB64, keyHandle).pipe(
          switchMap(() => {
            if (otherTokens.length === 0) {
              return of({ groupIdB64, epoch: 0, deviceWelcomes: [] as DeviceWelcomeDto[], mlsGroupInfo: undefined as string | undefined });
            }
            ...
            return this.mlsService.addMembers(groupIdB64, keyHandle, keyPackagesB64).pipe(
              switchMap(commitOut => {
                ...
                return this.mlsService.exportGroupInfo(groupIdB64, keyHandle).pipe(
                  map(mlsGroupInfo => ({ groupIdB64, epoch: commitOut.epoch, deviceWelcomes, mlsGroupInfo }))
                );
              })
            );
          })
