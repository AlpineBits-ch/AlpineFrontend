 ...
 *
 * <p>In a browser `enumerateDevices()` returns real ids with blank labels until the page holds a media
 * permission, and the adapter substitutes "Microphone 2" rather than rendering a nameless row. This menu
 * is where most people change device, so a numbered list with no explanation reads as a bug rather than
 * as a permission nobody has granted yet.</p>
 *
 * <p>Both directions are asserted, plus the case that would be the tempting mistake: with no devices
 * listed there is nothing for the line to be about, and it must not appear on its own.</p>
 */
