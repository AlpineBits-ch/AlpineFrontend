 ...
 *
 * <p><b>The bug this covers is an omission, which is the hardest kind to see in a component.</b>
 * `WebMlsEngine` restores the other tab's blob on its next command, so from the adapter's point of view
 * the takeover is complete - and the tab still shows "encryption unavailable" until it is reloaded,
 * because the launch sequence ran once, at boot, against an engine that refused everything. Nothing in
 * the adapter, the service or the component connected the two, and nothing failed while they were
 * disconnected.</p>
 *
 * <p>Tested here rather than through `MainPageComponent`, for the reason `mls-launch.ts` and
 * `sign-out.ts` are: the whole of the behaviour is control flow, and it would otherwise live invisibly
 * inside a component nothing can construct without half the application.</p>
 */
