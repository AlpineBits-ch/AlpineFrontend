 ...
 *
 * <p>`crates/venta-crypto/src/mls.rs` and `crates/venta-crypto/src/wasm.rs` both say, in comments,
 * that their `mls_init_storage` refusals are worded for a TypeScript classifier and that only one of
 * the two cases may wipe a device's groups. The Rust side has a test that its own refusal survives to
 * the adapter. Nothing on this side used to check anything, so the classifier did not exist and every
 * failure - including a keychain that was merely slow to start - wiped.</p>
 *
 * <p>This file is the mirror. It reads the Rust sources off disk rather than trusting a copied
 * comment, so rewording an engine error breaks a test here instead of silently turning transient
 * faults back into wipes.</p>
 */
 ...
 *
 * <p>Not `process.cwd()`: a runner is free to change it, and a contract check that silently found no
 * file would pass. Not a fixed number of `..` either - the test builder bundles specs, so
 * `import.meta.url` need not sit at the source path. Copied from `platform-boundary.spec.ts`, which
 * had to solve this first.</p>
 */
 ...
 *
 * <p>Rust's `\` line continuation drops the newline <b>and</b> the following indentation, so
 * `"a - b \\\n     c"` is the string `"a - b c"`. Every message in the engine is written that way
 * because they are all longer than a line, and a naive substring search for the runtime text would
 * therefore never match the source. Undoing the continuation is what makes this check about the
 * message the user's client receives rather than about how it happens to be typeset.</p>
 */
