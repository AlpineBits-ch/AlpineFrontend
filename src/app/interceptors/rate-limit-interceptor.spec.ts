...
describe('jittered', () => {
     ...
     *
     * <p>It used to be a flat 250ms tail, so a hundred requests parked behind the same 429 all
     * resumed inside a quarter second - 400 requests per second at a bucket that allows 50. The
     * landing zone now widens with the wait the server asked for.</p>
     */
    ...
    // The wait is already clamped to 30s; without a ceiling on the spread the jitter could double
    // that into a minute-long hang.
...
/**
 * Whether the backoff relieves a storm or joins it.
 *
 * <p>The evidence that prompted this: a conversation's message page, whose call site already has a
 * correct "already loading" guard, still appeared three times on the wire. One logical request
 * became MAX_ATTEMPTS real ones. These assert the count, not the outcome.</p>
 */
describe('rateLimitInterceptor amplification', () => {
    beforeEach(() => {
        _resetRateLimitState();
        // The interceptor compares `Date.now()` against timers it schedules itself, so both have
        // to move together or the gate never opens.
        vi.useFakeTimers();
     ...
     *
     * <p>It used to be read once, so a request parked behind a one-second wait went out on
     * schedule even when a 429 arriving in the meantime had pushed the gate five seconds further
     * out. The requests most likely to be parked were released into a bucket already known to be
     * empty.</p>
     */
