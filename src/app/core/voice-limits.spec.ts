/**
 * Reading a room's `limits` block.
 *
 * <p>The three things guide section 8 asks a client to do with it: draw denominators, disable a
 * control that would only be refused, and clamp the quality picker. None of them is an error path -
 * `none` is a rung, a full publisher list is a queue, and both are sentences rather than failures.
 * </p>
 */
...
describe('the video ceiling', () => {
    ...
    /**
     * `none` is answerable from the room snapshot alone, before the entitlement snapshot that
     * carries the ladder has landed. A room that only reads as audio-only once a second request
     * returns shows a live camera button for as long as that takes.
     */
...
describe('whether video may start', () => {
    ...
    /**
     * Stopping is never blocked. A ceiling that arrived after the publish did must not strand a
     * live share behind a stop button that no longer works.
     */
...
describe('the sentences', () => {
    ...
    /**
     * Negative, and the rule that matters most: a key this build has never heard of gets no
     * sentence rather than a raw code. The reason sentence still stands on its own.
     */
