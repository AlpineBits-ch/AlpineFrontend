...
import {
    BILLING_ERROR_TRANSLATION_KEYS,
...
describe('billingErrorCopy - the named refusals', () => {
    ...
    it('covers the codes the contract added after the service layer was written', () => {
        expect(billingErrorCopy(problem(403, {code: 'not_the_payer'})).key)
            .toBe('BILLING.ERROR.NOT_THE_PAYER');
        expect(billingErrorCopy(problem(409, {code: 'subscription_lapsed'})).key)
            .toBe('BILLING.ERROR.SUBSCRIPTION_LAPSED');
    });

    it('offers the change-plan wording rather than the code for an existing subscription', () => {
        expect(billingErrorCopy(problem(409, {code: 'already_subscribed'})).key)
            .toBe('BILLING.ERROR.ALREADY_SUBSCRIBED');
    });
});
...
describe('billingErrorCopy - what it does with what it does not know', () => {
    ...
    it('degrades a response that is not problem+json at all', () => {
        expect(billingErrorCopy(problem(500, '<html>502 Bad Gateway</html>')).key)
            .toBe(BILLING_GENERIC_ERROR_KEY);
    });
