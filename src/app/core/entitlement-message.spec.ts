...
describe('entitlementReasonKey', () => {
    ...
    /**
     * The cheapest possible proof that `boundBy` is needed: without it there is one sentence for two
     * opposite facts, and a paying member is eventually told their own plan limited them.
     */
...
describe('describeEntitlementLimit', () => {
    ...
    /**
     * An operator ceiling is what this box will do rather than what anyone paid for, and it is the
     * one code a self-hosted instance emits. Nothing that can be bought changes it.
     */
...
describe('describeEntitlementDenial', () => {
    ...
    /**
     * The two statuses the server never uses for this: 429 is retried three times by the rate-limit
     * interceptor with the body long gone, and 401 signs the user out.
     */
...
describe('uploadFailureKey', () => {
    ...
    /** A 413 is the same fact one layer lower down, and used to read as "the network died". */
...
describe('the key catalogue', () => {
    ...
    /**
     * The export `i18n-keys.spec.ts` checks against en.json. Losing an entry from it would leave the
     * table unguarded again without anything failing here, so the count is asserted too.
     */
...
/**
 * The third module empty state. "The owner turned it off" and "the plan does not include it" are
 * the same absence from what is effective, and only one of them has an upgrade next to it.
 */
describe('moduleNotInPlanCopy', () => {
    ...
    /**
     * A module added after this build still has to render a sentence. The fallback is written to
     * be true of any module rather than pretending to know what this one is.
     */
    ...
    /**
     * The reason these are a table rather than `` `${module}.NOT_IN_PLAN_TITLE` ``: a computed key
     * matches no pattern in `i18n-keys.spec.ts`, so a missing entry renders raw to a user with
     * every test green.
     */
...
/**
 * The same two server fields that drive a degradation's button drive the one on a settings screen,
 * which is why there is one function rather than two that drift.
 */
