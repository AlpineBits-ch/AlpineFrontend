...
describe('the disclaimer that has to sit next to a balance', () => {
    ...
    it('renders the server sentence for a key this build has never seen', () => {
        const copy = creditDisclaimerCopy(
            'Credits have no cash value.', 'credit.disclaimer.v2', () => false);
    ...
    it('falls back to its own sentence rather than showing a balance with nothing beside it', () => {
        ...
        expect(creditDisclaimerCopy(undefined, '   ', () => true).key)
            .toBe(CREDIT_DISCLAIMER_FALLBACK_KEY);
    });
});
...
describe('what a purchase says it did', () => {
    ...
    it('names the day a queued purchase starts, and never says it is active', () => {
        const copy = creditPurchaseCopy(purchase({
            wasQueued: true,
    ...
    it('has a sentence for a grant with no end date', () => {
        expect(creditPurchaseCopy(purchase({expiresAt: null})).bodyKey)
            .toBe('BILLING.CREDIT.PURCHASE.ACTIVE_BODY_NO_END');
        expect(creditPurchaseCopy(purchase({wasQueued: true, expiresAt: null})).bodyKey)
            .toBe('BILLING.CREDIT.PURCHASE.QUEUED_BODY_NO_END');
    });
});
...
describe('a refused spend', () => {
    ...
    it('is explained in the service own words', () => {
        const copy = creditErrorCopy(new HttpErrorResponse({
            status: 400,
            error: {
    ...
    it('apologises generically for a failure that carried no sentence at all', () => {
        expect(creditErrorCopy(new HttpErrorResponse({status: 500})).key)
            .toBe(CREDIT_GENERIC_ERROR_KEY);
    ...
    it('renders neither the code nor the status', () => {
        const copy = creditErrorCopy(new HttpErrorResponse({
            status: 400, error: {code: 'unknown_sku'},
        }));
