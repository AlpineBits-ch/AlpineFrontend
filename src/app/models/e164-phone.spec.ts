...
/**
 * The client half of a rule the server also enforces, so the cases here are taken from the server's
 * own test file rather than invented.
 *
 * <p>Two of them are load-bearing rather than routine, and both are the kind of thing a later
 * "helpful" edit removes:</p>
 *
 * <ul>
 *   <li><b>`00` is refused, not rewritten.</b> Turning `0041...` into `+41...` is right in most of
 *       the world and wrong in enough of it to produce a stranger's number. Nothing verifies the
 *       result, so a rewrite nobody looked at is a payment sent to the wrong person.</li>
 *   <li><b>A stray letter is a refusal, not something to strip.</b> Dropping it yields a different
 *       number that still looks entirely plausible.</li>
 * </ul>
 */
describe('checkE164', () => {
