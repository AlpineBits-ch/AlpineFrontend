 ...
 *
 * <p>`source` is offered on every rung on purpose - at picker time this client genuinely cannot know
 * how tall the chosen display is, so disabling it would be guessing at a mapping the server publishes
 * a ladder to keep out of here. By the time geometry is solved that is no longer true: the picker has
 * returned the source's real dimensions, so the rung's own `maxHeight` can be applied to a number
 * rather than to a guess. Without this a 4K display shared as `source` publishes 4K on a 720p30 rung,
 * and the server's clamp cannot help - it selects a simulcast layer, and a publisher sending one
 * encoding has no layer to select.</p>
 */
describe('solveGeometry against a granted rung', () => {
    ...
    it('leaves source alone when no rung has been resolved', () => {
        // The documented default everywhere on this path: nothing said means offer it and let the
        // server answer. Pinned so threading a ceiling through cannot quietly become a clamp on
        // instances that publish no limits at all.
    ...
    it('takes whichever of the box and the rung is smaller', () => {
        // The preset outlives the room it was chosen in, so this pairing is reachable: 2160p saved
        // on one server, carried into a room that grants 1080p60.
        expect(solveGeometry(3840, 2160, '2160p', RUNG_1080P60))
            .toEqual({width: 1920, height: 1080});
    });

    /**
     * `none` is audio-only, and the caller answers it by not publishing at all - see
     * `videoPublishBlock`. Reaching here with it must not produce a 2x2 capture, which is what
     * treating a zero ceiling as a height limit would do.
     */
